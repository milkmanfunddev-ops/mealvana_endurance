...
const events = [
  // NATIONAL CHAMPIONSHIPS
  {
    ...
    event_type: 'swimming',
    event_subtype: 'custom',  // Multiple distances
...
async function insertEvent(event) {
  const url = `${SUPABASE_URL}/rest/v1/public_events`;
...
async function checkExists(external_id, source) {
  const url = `${SUPABASE_URL}/rest/v1/public_events?external_id=eq.${external_id}&source=eq.${source}&select=id`;
...
async function main() {
  ...
  for (const event of events) {
    try {
      ...
      const exists = await checkExists(event.external_id, event.source);
      ...
      if (response.ok) {
        ...
        console.error(`   Status: ${response.status}, Message: ${errorText}`);
        errorCount++;
      }
    } catch (err) {
