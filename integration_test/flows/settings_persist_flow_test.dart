...
///
/// Auth:
///   Reuses session from `_ensureAuthenticated` (same pattern as green
///   sibling flows). Self-skips if no session and no credentials provided.
///
/// Run:
///   patrol test \
  ...
  patrolTest(
    ...
    ($) async {
      ...
      if (!await _ensureAuthenticated($)) {
        markTestSkipped(
          'Could not reach the main screen: no session and no '
      ...
      await $(const ValueKey('settings.profile_row')).waitUntilVisible(
        timeout: const Duration(seconds: 20),
      );
      ...
      await $(const ValueKey('settings.body_composition_row')).scrollTo().tap(
            settlePolicy: SettlePolicy.noSettle,
          );
      await $.pump(const Duration(milliseconds: 400));

      // Wait for the weight field to appear (screen calls userRepository async).
      await $(const ValueKey('body_composition.weight_field')).waitUntilVisible(
        timeout: const Duration(seconds: 20),
      );
      ...
      // be visible.
      await $(const ValueKey('nutrition_profile.save_button')).waitUntilVisible(
        timeout: const Duration(seconds: 10),
      );
      await $(const ValueKey('nutrition_profile.save_button')).tap(
        settlePolicy: SettlePolicy.noSettle,
      );
      ...
      await $(const ValueKey('nutrition_profile.back_button')).waitUntilVisible(
        timeout: const Duration(seconds: 10),
      );
      await $(const ValueKey('nutrition_profile.back_button')).tap(
        settlePolicy: SettlePolicy.noSettle,
      );
      await $.pump(const Duration(milliseconds: 400));
      ...
      expect(weightFinderAfter, findsOneWidget,
          reason: 'weight field must contain an EditableText after reload');
      final editableTextAfter = $.tester.widget<EditableText>(weightFinderAfter);
      expect(
        editableTextAfter.controller.text,
        sentinelStr,
        reason: 'Weight should persist as $sentinelStr after save + re-open. '
...
// ---------------------------------------------------------------------------
// Auth helper (same pattern as formula_create_pin_flow_test.dart)
// ---------------------------------------------------------------------------
