...
void main() {
  patrolTest(
    ...
    ($) async {
      ...
      // cause pumpAndSettle to sit for minutes. _ensureAuthenticated uses
      // explicit waitUntilVisible gates instead.
      ...
      await $(const ValueKey('formula_kit.editor_name')).waitUntilVisible(
        timeout: const Duration(seconds: 20),
      );
      ...
      await $(const ValueKey('formula_kit.editor_add_food')).waitUntilVisible(
        timeout: const Duration(seconds: 20),
      );
      await $(const ValueKey('formula_kit.editor_add_food')).tap(
        settlePolicy: SettlePolicy.noSettle,
      );
      await $.pump(const Duration(milliseconds: 400));
      ...
      await $(const ValueKey('swap_food.search_field')).enterText(
        'gel',
        settlePolicy: SettlePolicy.noSettle,
      );
      ...
      await $(firstFoodCard).waitUntilVisible(
      ...
      await $(const ValueKey('formula_kit.editor_row_0')).waitUntilVisible(
        timeout: const Duration(seconds: 10),
      );
      ...
      await $(const ValueKey('formula_kit.editor_save')).waitUntilVisible(
        timeout: const Duration(seconds: 10),
      );
      await $(const ValueKey('formula_kit.editor_save')).tap(
        settlePolicy: SettlePolicy.noSettle,
      );
...
/// Returns true once the main screen is reachable (authenticated).
///
/// Uses [fuel_timeline.settings] as the "on main screen" sentinel because
/// [calendar.create_activity_fab] was removed when Activities + Nutrition were
/// merged into the single Fuel Timeline tab (FuelTimelineScreen).
///
/// Polls for up to 90 s for EITHER:
///   • [sentinel] — already signed in, app loaded to the main screen, OR
///   • [welcomeLogIn] — not signed in, welcome screen shown.
///
/// Polling avoids pumpAndSettle, which hangs indefinitely on persistent
/// [CircularProgressIndicator]s during startup.
Future<bool> _ensureAuthenticated(PatrolIntegrationTester $) async {
  const sentinel = ValueKey('fuel_timeline.settings');
  const welcomeLogIn = ValueKey('welcome.log_in_button');

  // Poll every 500 ms for up to 90 s for sentinel or welcome screen.
  ...
  for (var i = 0; i < 180; i++) {
    ...
    if ($(welcomeLogIn).exists) {
      welcomeFound = true;
      break;
    }
  }

  // Already on the main screen — no login needed.
  if (sentinelFound) return true;

  // Welcome screen shown but no credentials — cannot proceed.
  if (!welcomeFound || _loginEmail.isEmpty || _loginPassword.isEmpty) {
    return false;
  }

  // Sign in via email.
  await $(welcomeLogIn).tap(settlePolicy: SettlePolicy.noSettle);
  await $.pump(const Duration(milliseconds: 300));
  ...
  await $(const ValueKey('login.email_field')).enterText(
    _loginEmail,
    settlePolicy: SettlePolicy.noSettle,
  );
  await $.pump(const Duration(milliseconds: 200));
  await $(const ValueKey('login.password_field')).enterText(
    _loginPassword,
    settlePolicy: SettlePolicy.noSettle,
  );
  await $.pump(const Duration(milliseconds: 200));
  ...
  await $(sentinel).waitUntilVisible(timeout: const Duration(seconds: 60));
