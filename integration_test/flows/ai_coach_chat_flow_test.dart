/// Jade AI-coach chat flow under **Patrol** — open /jade, send one short
...
/// and the JadeCoachBanner (the only in-UI entry, `jade.coach_banner`) is not
...
///     → chat screen renders (jade.chat_screen) — else skip
///     → count existing user/assistant bubbles
///     → type 'hi' (jade.input_field) → send (ValueKey('send'))
///     → user bubble count grows (send committed)
///     → poll up to 60 s for the assistant bubble count to grow
///       (jade.message_assistant_* — a REAL round-trip through the jade-chat
///        edge function, not just a button existing)
///     → back out of the chat.
///
/// Note: an empty conversation may trigger Jade's proactive opener, which
...
///
/// Run (dev only):
///   patrol test --target integration_test/flows/jade_chat_flow_test.dart \
///     --flavor dev \
...
Finder _userBubbles() => _bubbles('jade.message_user_');
Finder _assistantBubbles() => _bubbles('jade.message_assistant_');

void main() {
  patrolTest(
    'jade chat — send a message and receive an assistant response',
    ($) async {
      if (TestConfig.isProd) {
        markTestSkipped(
          'Jade chat calls the jade-chat edge function on every send — '
      ...
      // ---- 1. Open /jade ----------------------------------------------------
      if ($(const ValueKey('jade.coach_banner')).exists) {
        await $(
          const ValueKey('jade.coach_banner'),
      ...
      for (var i = 0; i < 30; i++) {
        await $.pump(const Duration(milliseconds: 500));
        if ($(const ValueKey('jade.chat_screen')).exists) {
          chatVisible = true;
          break;
        }
      }
      if (!chatVisible) {
        markTestSkipped(
          'Jade entry not available in this flavor '
      ...
      await $(
        const ValueKey('jade.input_field'),
      ...
      try {
        ...
        // ignore: avoid_print
        print('jade_chat back-navigation cleanup failed (non-fatal): $e');
      }
    },
