...
async function getFoodsForPhase(
  ...
  const { data: foods, error: foodsError } = await supabase
    ...
    .select(`
      ...
      product_type, brand_id, is_electrolyte, to_exclude_from_solver
    `)
  ...
  return foods
    .filter((f: any) => {
      const isDisliked = dislikedFoods.has(f.id) ||
        dislikedFoods.has(f.name?.toLowerCase());
    ...
    .map((f: any) => {
      ...
      if (likedFoods.has(f.id) || likedFoods.has(f.name?.toLowerCase())) {
        preference_score = 2.0; // Liked
      } else if (willTryFoods.has(f.id) || willTryFoods.has(f.name?.toLowerCase())) {
...
async function getElectrolyteFoods(
  ...
  return electrolytes
    .filter((e: any) => {
      // Only use liked or willing-to-try electrolytes
      return likedFoods.has(e.id) || likedFoods.has(e.name?.toLowerCase()) ||
             willTryFoods.has(e.id) || willTryFoods.has(e.name?.toLowerCase());
    })
    .map((e: any) => ({
      id: e.id,
      name: e.name,
      ...
      serving_unit_plural: e.serving_unit_plural,
...
async function postProcessPhase(
  ...
  if (needsWater && !hasPlainWater && !hasWaterWithElectrolytes) {
    ...
    const water = allFoods.find((f: Food) => f.name === 'Water' && f.serving_unit === 'fl oz');
    if (water) {
