...
serve(async (req)=>{
  ...
  try {
    ...
    // 2. Parse request
    const { device_id, activity, activity_id, operation } = await req.json();
    ...
    if (operation === 'create') {
      ...
      const { data, error } = await supabaseClient.from('activities').update({
        activity_type: activity.activityType,
        title: activity.title,
        scheduled_date_time: activity.scheduledDateTime,
        ...
        // Timestamp
        updated_at: new Date().toISOString()
      }).eq('id', activity.id).eq('user_id', deviceData.id).select().single();
      if (error) throw error;
      result = data;
      ...
      const { data, error } = await supabaseClient.from('activities').delete().eq('id', activity_id).eq('user_id', deviceData.id).select();
