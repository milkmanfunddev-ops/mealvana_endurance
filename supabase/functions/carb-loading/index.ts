...
const RACE_DISTANCES = {
  half_marathon: { name: 'Half Marathon', distanceKm: 21.1, multiplier: 0.8 },
  marathon: { name: 'Marathon', distanceKm: 42.2, multiplier: 1.0 },
  '50k': { name: '50K', distanceKm: 50.0, multiplier: 1.1 },
  '50_mile': { name: '50 Mile', distanceKm: 80.5, multiplier: 1.2 },
  '100k': { name: '100K', distanceKm: 100.0, multiplier: 1.3 },
  '100_mile': { name: '100 Mile', distanceKm: 161.0, multiplier: 1.4 },
} as const;

// Training volume multipliers
const TRAINING_VOLUMES = {
  low: { name: 'Low', description: '<30 miles/week', multiplier: 0.9 },
  moderate: { name: 'Moderate', description: '30-50 miles/week', multiplier: 1.0 },
  high: { name: 'High', description: '>50 miles/week', multiplier: 1.1 },
} as const;
...
type TrainingVolume = keyof typeof TRAINING_VOLUMES;

interface CarbLoadingRequest {
  raceDate: string;
  raceDistance: RaceDistance;
  trainingVolume: TrainingVolume;
  bodyWeightPounds: number;
  userId?: string;
}
...
interface CarbLoadingPlanData {
  dayMinusTwo: DayPlan;
  dayMinusOne: DayPlan;
  raceDay: DayPlan;
  bodyWeightKg: number;
  targetCarbsGPerKg: number;
  notes: string;
}

interface CarbLoadingResponse {
  success: boolean;
  plan?: CarbLoadingPlanData;
  error?: string;
  metadata: {
...
function calculateCarbLoadingPlan(request: CarbLoadingRequest): CarbLoadingPlanData {
  ...
  const targetCarbsGPerKg = CARB_LOADING_CONFIG.baseCarbsPerKg *
                           volumeInfo.multiplier *
                           distanceInfo.multiplier;

  // Calculate daily plans
  const dayMinusTwo = calculateDayPlan(
    'Day -2',
    bodyWeightKg,
    targetCarbsGPerKg * CARB_LOADING_CONFIG.dayMinusTwoIntensity,
    false
  );

  const dayMinusOne = calculateDayPlan(
    'Day -1',
    bodyWeightKg,
    targetCarbsGPerKg * CARB_LOADING_CONFIG.dayMinusOneIntensity,
    false
  );

  const raceDay = calculateDayPlan(
    'Race Day',
    bodyWeightKg,
    targetCarbsGPerKg * CARB_LOADING_CONFIG.raceDayIntensity,
    true
  );
...
function calculateDayPlan(
  ...
  // Macro distribution
  const carbsPercentage = isRaceDay ?
    CARB_LOADING_CONFIG.carbsPercentageRaceDay :
    CARB_LOADING_CONFIG.carbsPercentageNormal;
  const proteinPercentage = isRaceDay ?
    CARB_LOADING_CONFIG.proteinPercentageRaceDay :
    CARB_LOADING_CONFIG.proteinPercentageNormal;
  const fatPercentage = isRaceDay ?
    CARB_LOADING_CONFIG.fatPercentageRaceDay :
    CARB_LOADING_CONFIG.fatPercentageNormal;
...
function generatePlanNotes(raceDistance: RaceDistance, trainingVolume: TrainingVolume): string {
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request
    const requestData: CarbLoadingRequest = await req.json()
    ...
    const response: CarbLoadingResponse = {
      success: true,
      plan,
      metadata: {
        calculationTime,
