...
export async function runAction(v: VanaCtx, a: UiAction): Promise<ActionResult> {
  ...
  switch (a.type) {
    ...
    case 'set_day_slot': { // { date?, slot, source: 'plan'|'saved'|'library', id }
      ...
      else { const m = await getMeal(v, p.source, String(p.id)); if (!m) throw new Error(`meal not found: ${String(p.id)}`); plan.mustHaveNumbers(m); name = m.name; kcal = m.kcal; carbsG = m.carbsG; }   // a blank meal never goes on a day (mp-678, ticket 74)
