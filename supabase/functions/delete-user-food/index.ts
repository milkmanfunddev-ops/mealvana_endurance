...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: existingFood, error: fetchError } = await supabaseClient.from('user_foods').select('id, name').eq('id', requestData.food_id).eq('device_id', requestData.device_id).single();
