...
import { isEntitled, requirePro } from '../../_shared/vana/entitlement.ts';
...
type Row = { user_id: string; active_until: string | null; period_type: string | null };
...
Deno.test('a trial row (period_type TRIAL, active_until in seven days) passes', async () => {
  ...
  assertEquals(log, [{ table: 'user_entitlements', cols: 'active_until, period_type', user: USER }]);
});
