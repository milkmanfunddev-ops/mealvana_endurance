...
const rnd = (v, p = 1)=>Math.round(v * p) / p;
function validateInputs(inputs) {
  ...
  if (inputs.gutTraining !== undefined && ![
    'low',
    'moderate',
    'high'
  ].includes(inputs.gutTraining)) {
    errors.push('gutTraining must be one of: low, moderate, high');
  }
  if (inputs.giSensitivity !== undefined && ![
    'normal',
    'sensitive'
  ].includes(inputs.giSensitivity)) {
  ...
  if (inputs.sweatRate !== undefined && ![
    'low',
    'moderate',
    'high'
  ].includes(inputs.sweatRate)) {
    errors.push('sweatRate must be one of: low, moderate, high');
  }
  if (inputs.intervalMinutes !== undefined && ![
    20,
    30
  ].includes(inputs.intervalMinutes)) {
...
function envMultipliers(tempF, humidity, sweatRate) {
...
// ---- ACSM (optional info)
function acsmFromPace(weightKg, paceMinPerKm, durationH) {
  ...
  return {
    VO2_ml_kg_min: rnd(VO2, 100),
    MET: rnd(MET, 100),
    kcal: rnd(kcal, 1)
  };
}
// ---- Targets per your analyses/guidelines
function calcTargets(inp) {
  ...
  return {
    ...
    after: {
      ...
      fluid_ml: afterFluids,
      sodium_mg: afterSodium
    }
  };
}
...
function getFallbackFoods(foods) {
  const gel = foods.find((f)=>f.name.toLowerCase().includes('gel') && f.during_run_suitable) ?? foods.find((f)=>f.during_run_suitable && f.carbs_per_serving && f.carbs_per_serving >= 20);
  const drink = foods.find((f)=>f.name.toLowerCase().includes('sport') && f.name.toLowerCase().includes('drink')) ?? foods.find((f)=>f.during_run_suitable && f.fluid_ml_per_serving && f.fluid_ml_per_serving > 200);
  const recovery = foods.find((f)=>f.name.toLowerCase().includes('chocolate') && f.name.toLowerCase().includes('milk')) ?? foods.find((f)=>f.carbs_per_serving && f.protein_per_serving && f.carbs_per_serving >= 20 && f.protein_per_serving >= 5);
  return {
    gel,
    drink,
    recovery
...
async function fetchPrefs(deviceId) {
  if (!deviceId) return {};
...
async function fetchAfterEligibleIds() {
...
function scoreDuring(f, prefs, t, gi) {
  const pref = prefs[f.name];
  if (pref === "dislike") return 0;
...
// ---- Selection / Assembly
function pickTop(foods, prefs, phase, t, gi, afterIds) {
  let eligible;
  if (phase === "before") {
  ...
  }).filter((x)=>x.score > 0).sort((a, b)=>b.score - a.score).slice(0, phase === "during" ? 4 : 6); // keep a small set
  return scored;
}
function buildPre(top, targetCarbs, sodiumOpt) {
  const items = [];
  let carbs = 0;
...
function buildDuring(top, t, durationMin, intervalMin, gi, fallbackFoods) {
  const events = [];
  ...
  const roster = top.map((x)=>x.ref).filter((f)=>gi === "normal" ? true : (f.calories_per_serving ?? 0) <= SCORING_CONFIG.during.gi_sensitive_calorie_limit).slice(0, 3);
...
function getBasicFallbackPlan(weightKg, durationMin) {
  const durationH = durationMin / 60;
  return {
    before: {
      items: [
        {
          name: "Oatmeal (fallback)",
          servings: 1,
          carbs_g: 30
        }
      ],
    ...
    during: {
      ...
      events: Array.from({
        length: Math.ceil(durationMin / 30)
      }, (_, i)=>({
          at_min: (i + 1) * 30,
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // Data fetch
    const [foods, prefs, afterIds] = await Promise.all([
      getFoodsCached(),
      fetchPrefs(body.deviceId),
      getAfterIdsCached()
    ]);
    // Get fallback foods for edge cases
    ...
    const plan = {
      before: {
        ...prePlan,
        fluid_ml_sip: targets.before.fluid_ml
      },
      during: duringPlan,
      after: afterPlan
    };
    ...
    if (body.debug) {
      response.debug = {
        ...
        top_scores: {
          before: topBefore.slice(0, 3).map((x)=>({
              name: x.ref.name,
              score: Math.round(x.score)
            })),
          during: topDuring.slice(0, 3).map((x)=>({
              name: x.ref.name,
              score: Math.round(x.score)
            })),
          after: topAfter.slice(0, 3).map((x)=>({
              name: x.ref.name,
              score: Math.round(x.score)
            }))
        },
      ...
      response.selections = {
        before_top_choices: topBefore.map((x)=>({
            name: x.ref.name,
            score: Math.round(x.score)
          })),
        during_top_choices: topDuring.map((x)=>({
            name: x.ref.name,
            score: Math.round(x.score)
          })),
        after_top_choices: topAfter.map((x)=>({
            name: x.ref.name,
            score: Math.round(x.score)
          }))
      };
