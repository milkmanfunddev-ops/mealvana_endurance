...
interface SaveCarbLoadingPlanRequest {
  device_id: string;
  plan?: any; // Full plan object with protocol info
  plan_id?: string; // For delete operations
  operation: 'create' | 'update' | 'delete';
}

// Helper function to get carb protocol for a specific day
function getCarbProtocolForDay(protocolDays: number, daysBeforeRace: number): number {
  if (protocolDays === 2) {
    // 2-day protocol
...
serve(async (req) => {
  ...
  try {
    ...
    // 2. Parse request
    const { device_id, plan, plan_id, operation } = await req.json() as SaveCarbLoadingPlanRequest;
    ...
    if (operation === 'create') {
      ...
      const { data: days } = await supabaseClient
        .from('carb_loading_days')
        .select('id')
        .eq('carb_loading_plan_id', plan_id);
