...
serve(async (req)=>{
  ...
  try {
    ...
    let query = supabaseClient.from('carb_loading_foods').select(`
        *,
    ...
    const { data: mealTypes, error: mealTypesError } = await supabaseClient.from('meal_types').select('*').order('id', {
      ascending: true
    });
    if (mealTypesError) {
    ...
    const formattedFoods = foods?.map((food)=>({
        ...
        meal_type_ids: food.carb_loading_food_meal_types?.map((mt)=>mt.meal_type_id) || []
      })) || [];
