...
serve(async (req)=>{
  ...
  try {
    ...
    // 2. Parse request
    const { device_id, event, event_id, operation } = await req.json();
    ...
    if (operation === 'create') {
      ...
      if (event.activityId) {
        const { data: activityData, error: activityError } = await supabaseClient.from('activities').select('id').eq('id', event.activityId).eq('user_id', deviceData.id).single();
      ...
      const { data, error } = await supabaseClient.from('events').update({
        activity_id: event.activityId || null,
        event_type: event.eventType,
        event_subtype: event.eventSubtype,
        event_name: event.eventName,
      ...
      const { data, error } = await supabaseClient.from('events').delete().eq('id', event_id).select().single();
      if (error) throw error;
      result = data;
    }
