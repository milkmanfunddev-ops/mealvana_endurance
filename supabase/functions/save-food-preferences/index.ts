...
serve(async (req) => {
  ...
  try {
    ...
    const { device_id, food_preferences } = await req.json() as SaveFoodPreferencesRequest;
    ...
    const { data: savedPreferences, error: fetchError } = await supabaseClient
