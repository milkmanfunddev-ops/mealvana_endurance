...
interface SaveUserFoodRequest {
  ...
  category_ids: number[]; // Array of category IDs (1=before_run, 2=during_run, 3=after_run)
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const requestData: SaveUserFoodRequest = await req.json();
    ...
    const { data: userFoodData, error: userFoodError } = await supabaseClient
      .from('user_foods')
      .upsert({
        id: requestData.id,
        device_id: requestData.device_id,
    ...
    if (userFoodError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to save user food', details: userFoodError.message }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    const { error: deleteError } = await supabaseClient
      ...
      .delete()
      .eq('user_food_id', requestData.id);
    ...
    if (categoryError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to save food categories', details: categoryError.message }),
        {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
