...
--
-- Browse search ("salmon" on a vegetarian account) returned eight meals with no
-- salmon in them. With no embedding, search_meals scored every row by trigram
-- similarity against search_text, which carries the library's `why` (the
-- research note), and filtered nothing: the rows the note mentioned salmon in
-- ranked first.
--
-- Now, when p_query is set and p_embedding is not (the app's Browse search, and
-- Vana's search whenever it has no embedding), a row is returned only when every
-- word of the query is in its name or its ingredients, and it is scored on those
-- two alone. The embedding path (Vana's semantic search) is unchanged. The edge
-- functions apply the same predicate (`matchesMealText` in _shared/vana/meals.ts).
--
-- Signature and return columns are unchanged from
-- 20260916141000_search_meals_current_photo.sql; only the body changes.
-- Idempotent. Dev first; prod takes it with the meal-planning cutover.

-- One transaction: between the drop and the create there is no search_meals, and
-- every Meals tab and Vana search would fail.
begin;
...
returns table (
  ...
  photo_url text, photo_credit text, photo_credit_url text
)
  ...
  lib as (
    select 'library'::text as source, m.id, m.name, m.meal_type, m.contexts, m.batch, m.prep_minutes,
           ...
           ((case when p_embedding is not null and m.embedding is not null then 1 - (m.embedding <=> p_embedding)
                  when p_query is not null then greatest(similarity(m.name, p_query), word_similarity(p_query, m.name || ' ' || coalesce(m.ingredients, '')))
                  else 0.5 end)
           ...
           m.photo_url, m.photo_credit, m.photo_credit_url
    ...
    where m.is_active
      ...
      and (p_query is null or p_embedding is not null                         -- typed search: the meal, not its note
           or not exists (select 1 from regexp_split_to_table(lower(btrim(p_query)), '\s+') w
                           where w <> '' and strpos(lower(m.name || ' ' || coalesce(m.ingredients, '')), w) = 0))
  ),
  mine as (
    -- A saved Meal shows its library Meal's current photo, so a photo added
    -- later reaches meals the athlete already saved (ADR 0003, story 11).
    select 'saved'::text as source, s.id::text, s.name,
           ...
           l.photo_url, l.photo_credit, l.photo_credit_url
    ...
    where p_include_saved and s.user_id = p_user_id and not s.is_deleted
      ...
      -- Typed search: every word in the saved meal's name or its items.
      and (p_query is null or p_embedding is not null
           or not exists (select 1 from regexp_split_to_table(lower(btrim(p_query)), '\s+') w
                           where w <> '' and strpos(lower(s.name || ' ' || coalesce(
                             (select string_agg(coalesce(i->>'name', i->>'food_name', ''), ', ') from jsonb_array_elements(s.items) i), '')), w) = 0))
  )
