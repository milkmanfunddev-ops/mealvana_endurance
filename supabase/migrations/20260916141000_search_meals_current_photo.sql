-- search_meals returns the Meal's current Dish photo (ADR 0003).
--
-- Three columns appended — photo_url, photo_credit, photo_credit_url — so the
-- Meals tab, the rails, the swap screen and the shopping list read the same
-- photo the recipe screen does, from the same place.
--
-- The pipeline's image columns (image_url, image_mode, image_tiles,
-- image_credit, image_source_url, image_creator, image_license) are still
-- returned. The app no longer reads them; they stay on the wire so nothing
-- that still reads them breaks while the pipeline is frozen rather than deleted.
--
-- Nothing is removed or reordered: callers read columns by name. Only the return
-- type changes, so the signature is dropped and recreated (Postgres cannot alter
-- a function's OUT columns in place).
--
-- Requires 20260916140000_meal_photo_dish_or_nothing.sql (the photo_* columns).
