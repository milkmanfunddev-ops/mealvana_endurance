...
--          carb loading plans)
--
-- CRITICAL FIX: Events RLS uses events.user_id directly instead of requiring
--               an activities JOIN. This fixes events with activity_id = NULL
--               (standalone events) being invisible to everyone.
--
-- SECURITY NOTES:
...
-- ============================================================================
-- PHASE 2: EVENTS TABLE - Coach Access (FIXED: use events.user_id directly)
-- ============================================================================
-- CRITICAL FIX: The archived migration used an activities JOIN which fails
-- for events with activity_id = NULL (standalone events). Events have their
-- own user_id column - use it directly.
...
-- SELECT: Owner OR active coach (using events.user_id directly)
CREATE POLICY "events_select_policy" ON events FOR SELECT USING (
  -- Owner can view their own events (direct user_id check, works for all events)
  user_id = current_setting('app.user_id', true)::uuid
  -- OR coach with active relationship can view athlete's events
  OR EXISTS (
    SELECT 1 FROM coach_athlete_relationships
    WHERE coach_user_id = current_setting('app.user_id', true)::uuid
...
CREATE POLICY "events_insert_policy" ON events FOR INSERT WITH CHECK (
  ...
  user_id = current_setting('app.user_id', true)::uuid
  -- OR coach with active relationship can create events for athlete
  OR EXISTS (
    SELECT 1 FROM coach_athlete_relationships
    WHERE coach_user_id = current_setting('app.user_id', true)::uuid
      AND athlete_user_id = events.user_id
...
CREATE POLICY "events_update_policy" ON events FOR UPDATE USING (
  ...
  user_id = current_setting('app.user_id', true)::uuid
  -- OR coach with active relationship can update athlete's events
  OR EXISTS (
    SELECT 1 FROM coach_athlete_relationships
    WHERE coach_user_id = current_setting('app.user_id', true)::uuid
      AND athlete_user_id = events.user_id
