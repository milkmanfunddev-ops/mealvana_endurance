...
interface OpenFoodFactsResponse {
  ...
  product?: {
    ...
    nutriments?: {
      ...
      "sodium_100g"?: number;
      "salt_100g"?: number;
      // Per-serving values (note: different key names!)
...
interface NormalizedFood {
  ...
  nutrition_data_per?: string; // "serving" or "100g"

  // Source tracking
  api_source: "open_food_facts" | "usda" | "manual";
...
function getCachedResult(barcode: string): NormalizedFood | null {
...
function cacheResult(barcode: string, data: NormalizedFood): void {
  barcodeCache.set(barcode, { data, timestamp: Date.now() });
}

// Open Food Facts API integration
async function lookupOpenFoodFacts(barcode: string): Promise<NormalizedFood | null> {
  try {
...
async function lookupUSDA(barcode: string): Promise<NormalizedFood | null> {
  try {
    ...
    const normalized: NormalizedFood = {
      ...
      image_url: undefined, // USDA doesn't provide images
