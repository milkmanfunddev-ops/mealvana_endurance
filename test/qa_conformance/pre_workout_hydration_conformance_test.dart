...
void main() {
  ...
  group('pre-workout hydration conformance (${vectors.length} vectors)', () {
    for (final v in vectors) {
      ...
      test('[$status] $id', () {
        ...
        void chkFluid(String key, double? actual) {
          ...
          if (expected == null) {
            expect(
              ...
              isNull,
              reason:
                  '$id: $key must be null, not $actual (gate path states nothing). $why',
            );
          } else {
            expect(
              actual,
              isNotNull,
              reason: '$id: $key must be $expected, got null. $why',
            );
            expect(
              actual!,
              closeTo((expected as num).toDouble(), tolMl),
        ...
        if (!e.containsKey('hydrationCheckUsed') &&
            e['gateTriggered'] == true) {
          expect(
            out.hydrationCheckUsed,
            isNull,
        ...
        if (out.gateTriggered) {
          expect(
            out.tiers,
            isEmpty,
            reason: '$id: gate path emits an empty tiers array (inv 11)',
          );
        } else {
          final sum = out.tiers.fold<double>(0, (a, t) => a + t.fluidMl);
          expect(
            sum,
            closeTo(out.fluidMl!, tolMl),
            reason:
                '$id: sum(tiers[].fluidMl)=$sum must equal fluidMl=${out.fluidMl} (inv 2)',
          );
          for (final t in out.tiers) {
            expect(
              const <String>{'meal', 'snack', 'top_off'},
              contains(t.tier),
  ...
  group('pre-workout hydration invariants (property, grid 0..240 x BW 30..160)', () {
    test(
      'inv 1 — 0 <= fluidLowMl <= fluidMl <= fluidHighMl wherever non-null',
      () {
          ...
          for (final t in kLeadGrid) {
            for (final c in kAllChecks) {
              ...
              expect(
                o.gateTriggered,
                isFalse,
                reason: '$where: 90 min @ 22 C must not gate',
              );
              ...
              expect(
                o.fluidLowMl! >= 0,
                isTrue,
                reason: '$where: low ${o.fluidLowMl} < 0',
              );
              expect(
                o.fluidLowMl! <= o.fluidMl! + 1e-9,
                isTrue,
                reason: '$where: low ${o.fluidLowMl} > plan ${o.fluidMl}',
              );
              expect(
                o.fluidMl! <= o.fluidHighMl! + 1e-9,
                isTrue,
      ...
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          for (final c in kAllChecks) {
            ...
            expect(
              sum,
              closeTo(o.fluidMl!, 1e-9),
              reason:
                  'BW=$bw t=$t check=$c: tiers sum $sum != fluidMl ${o.fluidMl}',
            );
          }
        }
      }
    ...
    test(
      'inv 3 — fluidMl is non-decreasing in t on BOTH the pale and dark paths',
      () {
        for (final bw in kBodyWeights) {
          for (final c in <HydrationCheck>[
            HydrationCheck.pale,
            HydrationCheck.dark,
          ]) {
            ...
            for (final t in kLeadGrid) {
              ...
              expect(
                ...
                isTrue,
                reason:
                    'BW=$bw check=$c: fluidMl dropped from $prev to $cur at t=$t',
              );
              prev = cur;
            }
          }
    ...
    test(
      'inv 4 — pale-path continuity at t=120 and t=30 (|delta| < 1e-6 * BW)',
      () {
        const eps =
            1e-5; // small enough that the linear taper's own slope stays under 1e-6*BW
        for (final bw in kBodyWeights) {
          for (final b in <double>[kTRef, 30.0]) {
            ...
            expect(
              (above - below).abs() < 1e-6 * bw,
              isTrue,
              reason:
                  'BW=$bw: pale path is discontinuous at t=$b '
    ...
    test(
      'inv 5 — the dark-path step at T_REF is intentional: plan(120+) - plan(120-) == 4*BW',
      () {
        const eps = 1e-5;
        for (final bw in kBodyWeights) {
          final above = calc(
          ...
          expect(
            above - below,
            closeTo(kTopUpMlKg * bw, 1e-5 * bw),
            reason:
                'BW=$bw: dark step at T_REF is ${above - below}, expected ${kTopUpMlKg * bw}',
          );
        }
      },
    );
    ...
    test(
      'inv 8 — fluidHighMl == 12*BW for t >= 120; fluidHighMl <= 10*BW for t < 120',
      () {
        for (final bw in kBodyWeights) {
          for (final t in kLeadGrid) {
            for (final c in kAllChecks) {
              ...
              if (t >= kTRef) {
                expect(
                  high,
                  closeTo(kPlanCapMlKg * bw, 1e-9),
                  reason:
                      'BW=$bw t=$t check=$c: high $high != 12*BW ${kPlanCapMlKg * bw}',
                );
              } else {
                expect(
                  high <= 10.0 * bw + 1e-9,
                  isTrue,
                  reason:
                      'BW=$bw t=$t check=$c: high $high exceeds 10*BW ${10 * bw}',
                );
              }
            }
          }
    ...
    test(
      'inv 8a — the plan cap has teeth: fluidMl <= 12*BW, and dark high == 12*BW',
      () {
        for (final bw in kBodyWeights) {
          for (final t in kLeadGrid) {
            for (final c in kAllChecks) {
              final o = calc(bw, t, check: c);
              expect(
                o.fluidMl! <= kPlanCapMlKg * bw + 1e-9,
                isTrue,
                reason:
                    'BW=$bw t=$t check=$c: plan ${o.fluidMl} exceeds PLAN_CAP ${kPlanCapMlKg * bw}. '
            ...
            if (t >= kTRef) {
              ...
              expect(
                darkHigh < (7.5 + kTopUpHighMlKg) * bw - 1e-9,
                isTrue,
                reason:
    ...
    test(
      'inv 8b — above T_REF the band is byte-identical across all three hydrationCheck values',
      () {
        for (final bw in kBodyWeights) {
          for (final t in kLeadGrid.where((t) => t >= kTRef)) {
            ...
            for (final o in <PreWorkoutHydrationResult>[dark, unk]) {
              expect(
                ...
                pale.fluidLowMl,
                reason:
                    'BW=$bw t=$t: fluidLowMl moved with hydrationCheck (inv 8b)',
              );
              expect(
                o.fluidHighMl,
                pale.fluidHighMl,
                reason:
                    'BW=$bw t=$t: fluidHighMl moved with hydrationCheck (inv 8b)',
              );
              expect(
                o.regime,
                pale.regime,
                reason: 'BW=$bw t=$t: regime moved with hydrationCheck',
              );
              expect(
                o.targetBasis,
                pale.targetBasis,
            ...
            expect(
              dark.fluidMl! - pale.fluidMl!,
              closeTo(kTopUpMlKg * bw, 1e-9),
              reason:
                  'BW=$bw t=$t: dark must add exactly 4 ml/kg to the target',
            );
            ...
            expect(
              ...
              pale.fluidMl,
              reason:
                  'BW=$bw t=$t: unknown must be treated as pale above T_REF',
            );
            expect(
              unk.hydrationCheckUsed,
              'pale',
              reason:
                  'BW=$bw t=$t: hydrationCheckUsed echoes the NORMALISED value above T_REF',
            );
    ...
    test(
      'inv 8b (below T_REF) — all three checks produce identical output, and the RAW echo',
      () {
        for (final bw in kBodyWeights) {
          for (final t in kLeadGrid.where((t) => t < kTRef)) {
            final pale = calc(bw, t, check: HydrationCheck.pale);
            ...
            for (final o in <PreWorkoutHydrationResult>[dark, unk]) {
              expect(
                o.fluidMl,
                pale.fluidMl,
                reason: 'BW=$bw t=$t: check changed the sub-2h target',
              );
              expect(o.fluidLowMl, pale.fluidLowMl, reason: 'BW=$bw t=$t: low');
              expect(
                o.fluidHighMl,
                pale.fluidHighMl,
                reason: 'BW=$bw t=$t: high',
              );
              expect(o.regime, pale.regime, reason: 'BW=$bw t=$t: regime');
              expect(
                o.targetBasis,
                pale.targetBasis,
                reason: 'BW=$bw t=$t: targetBasis',
              );
              expect(
                o.tiers.length,
                pale.tiers.length,
                reason: 'BW=$bw t=$t: tier count',
              );
            }
            // Below T_REF the echo is the RAW value — no normalisation.
            expect(
              pale.hydrationCheckUsed,
              'pale',
              reason: 'BW=$bw t=$t: raw echo',
            );
            expect(
              dark.hydrationCheckUsed,
              'dark',
              reason: 'BW=$bw t=$t: raw echo',
            );
            expect(
              ...
              'unknown',
              reason:
                  'BW=$bw t=$t: below T_REF `unknown` is echoed RAW, never normalised to pale',
            );
    ...
    test(
      'inv 9 — clearance containment: the clearance-bound region sits inside the top-off tier',
      () {
        ...
        for (var bw = 30.0; bw <= 198.0; bw += 1.0) {
          ...
          expect(
            ...
            isTrue,
            reason:
                'BW=$bw: clearance crosses at t=$crossover, outside the top-off tier (<30)',
          );
        }
    ...
    test(
      'inv 11 — gate path is null/null/null with an empty tiers array, never 0',
      () {
        for (final bw in kBodyWeights) {
          for (final t in kLeadGrid) {
            for (final c in kAllChecks) {
              ...
              for (final temp in <double?>[22.0, null, 29.999]) {
                ...
                expect(
                  o.gateTriggered,
                  isTrue,
                  reason: '$where: gate must fire',
                );
                expect(
                  o.fluidMl,
                  isNull,
                  reason: '$where: fluidMl must be null, not 0',
                );
                expect(
                  o.fluidLowMl,
                  isNull,
                  reason: '$where: fluidLowMl must be null, not 0',
                );
                expect(
                  o.fluidHighMl,
                  isNull,
                  reason: '$where: fluidHighMl must be null, not 0',
                );
                ...
                expect(
                  o.hydrationCheckUsed,
                  isNull,
                  reason: '$where: hydrationCheckUsed',
                );
              }
              // ...and it must NOT fire once either leg of the AND is false.
              expect(
                calc(bw, t, check: c, durationMin: 60, tempC: 22).gateTriggered,
                isFalse,
                reason: 'BW=$bw t=$t: duration 60 is inclusive — must not gate',
              );
              expect(
                calc(bw, t, check: c, durationMin: 45, tempC: 30).gateTriggered,
                isFalse,
