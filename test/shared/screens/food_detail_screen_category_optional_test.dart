...
void main() {
  group('FoodDetailScreen category selection is optional', () {
    testWidgets('save button is enabled with no categories selected', (
      tester,
    ) async {
      ...
      expect(
        ...
        isNotNull,
        reason:
            'Save button should be enabled even with no categories selected',
      );
    });

    testWidgets('category selector shows optional label', (tester) async {
      final foodData = FoodDetailData(
        id: 'test-food-id',
        name: 'Gelato',
        carbsPerServing: 30,
      ...
      expect(find.text('Select all that apply (optional)'), findsOneWidget);
      expect(
