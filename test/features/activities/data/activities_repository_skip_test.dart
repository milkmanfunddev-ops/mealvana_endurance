...
void main() {
  ...
  setUp(() async {
    ...
    when(() => logger.info(any(),
        context: any(named: 'context'), data: any(named: 'data'))).thenReturn(null);
  ...
  group('Skip / Unskip write seam (G5)', () {
    test('skip writes status=skipped, clears actual_time, keeps planned_time',
        () async {
      final run = await seedPlannedRun();
      expect((await row(run.id)).plannedTime, plannedAt,
  ...
  group('provider re-sync merge preserves the two-time model', () {
    test('a title-only TP update keeps planned/actual time and measured kcal',
        () async {
      final run = await seedPlannedRun();
      ...
      await (database.update(database.activitiesTable)
            ..where((t) => t.id.equals(run.id)))
          .write(const db.ActivitiesTableCompanion(caloriesBurned: Value(1180)));
    ...
    test('a provider RESCHEDULE moves planned_time (scheduling owns it)',
        () async {
      final run = await seedPlannedRun();
