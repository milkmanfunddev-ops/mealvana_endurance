...
void main() {
  ...
  group('findActivityByFingerprint', () {
    test(
      'matches a user-created run on same type, day, and distance',
      () async {
    ...
    test(
      'does not match a provider-linked row (provider-key path owns those)',
      () async {
        await repository.insertActivity(
          buildActivity(
            title: 'Synced Run',
    ...
    test(
      ...
      () async {
        ...
        expect(
          rows.length,
          1,
          reason: 'cross-origin rows must reconcile to one',
        );
