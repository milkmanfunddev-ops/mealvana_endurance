...
void main() {
  ...
  group('CreditsController.build — initial load', () {
    ...
    test('state is AsyncData (not AsyncLoading) after build resolves', () async {
      when(() => repo.fetchWallet()).thenAnswer((_) async => _wallet100);
  ...
  group('CreditsController.refresh — re-fetch', () {
    ...
    test('refresh updates state with new wallet value from repository', () async {
      var callCount = 0;
      ...
      expect(container.read(creditsControllerProvider).value!.balance, 110,
          reason: 'refresh() must update state with the freshest wallet balance.');
    });

    test('refresh exposes AsyncError when repository throws on re-fetch', () async {
      var callCount = 0;
      when(() => repo.fetchWallet()).thenAnswer((_) async {
        callCount++;
      ...
      final state = container.read(creditsControllerProvider);
      expect(state, isA<AsyncError<CreditWallet>>(),
  ...
  group('CreditsController — edge cases', () {
    test('freePeriod="trial" wallet is surfaced correctly', () async {
      ...
      expect(wallet.balance, 0);
      expect(wallet.freePeriod, 'trial',
          reason: 'Free-trial users may have 0 balance but a non-null freePeriod.');
    });

    test('large credit balance is preserved exactly', () async {
      when(() => repo.fetchWallet()).thenAnswer(
        (_) async => const CreditWallet(balance: 999999),
      );
    ...
    test('multiple refreshes in sequence each update state', () async {
      ...
      await container.read(creditsControllerProvider.notifier).refresh(); // refresh → 100
      expect(container.read(creditsControllerProvider).value?.balance, 100);

      await container.read(creditsControllerProvider.notifier).refresh(); // refresh → 150
