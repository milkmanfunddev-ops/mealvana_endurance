...
void main() {
  ...
  group('EducationService.getContentGroups', () {
    ...
    test(
      'returns all-empty groups when repository returns empty list',
      () async {
    ...
    test('puts all-free content in freeVideos only', () async {
      when(() => mockRepo.getPublishedContent()).thenAnswer(
        (_) async => [
          _video(id: 'f1', contentType: 'free'),
          _video(id: 'f2', contentType: 'free'),
        ],
    ...
    test('puts all-pro content in proVideos only', () async {
      when(
        () => mockRepo.getPublishedContent(),
      ).thenAnswer((_) async => [_video(id: 'p1', contentType: 'pro')]);
    ...
    test(
      'preserves order from repository (no re-sorting by service)',
      () async {
        ...
        expect(
          groups.freeVideos.map((v) => v.id).toList(),
          ['first', 'second', 'third'],
  ...
  group('EducationController build', () {
    test('loads EducationContentGroups on build', () async {
      final content = [_video(id: 'v1', contentType: 'free')];
      when(
        () => mockRepo.getPublishedContent(),
      ).thenAnswer((_) async => content);

      final container = ProviderContainer(
        overrides: [educationRepositoryProvider.overrideWithValue(mockRepo)],
      );
