...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => coachService.getRelationshipById(_relationshipId),
    ).thenAnswer((_) async => _activeRelationship());
    when(
      () => coachService.getCurrentUserId(),
    ).thenAnswer((_) async => _coachUserId);
    when(
      () => coachService.getGeneralChatMessages(
    ...
    when(
      () => coachService.subscribeToConversation(
        coachUserId: any(named: 'coachUserId'),
        athleteUserId: any(named: 'athleteUserId'),
    ...
    when(
      () => coachService.markConversationAsRead(
        coachUserId: any(named: 'coachUserId'),
        athleteUserId: any(named: 'athleteUserId'),
      ),
    ).thenAnswer((_) async {});
    when(
      () => coachService.unsubscribeFromConversation(any()),
    ).thenAnswer((_) async {});
  });
  ...
  group('CoachChatController.build', () {
    test('loads messages and exposes relationship on initial build', () async {
      ...
      final state = container
          .read(coachChatControllerProvider(_relationshipId))
          .value!;
    ...
    test('produces error state when relationship is not found', () async {
      ...
      final result = await container
          .read(coachChatControllerProvider(_relationshipId).future)
          .then((_) => false)
          .catchError((_) => true);
      expect(
        result,
    ...
    test(
      'produces error state when current user is not part of relationship',
      () async {
        when(
          () => coachService.getCurrentUserId(),
  ...
  group('CoachChatController.sendMessage', () {
    test(
      ...
      () async {
        ...
        await container.read(
          coachChatControllerProvider(_relationshipId).future,
        );
    ...
    test(
      'on success: message moves from pending to confirmed messages',
      () async {
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          isEmpty,
          reason:
              'A null return from sendChatMessage must not be treated as success.',
        );
    ...
    test(
      'empty message text is ignored and produces no state change',
      () async {
        final container = _container();
        await container.read(
          coachChatControllerProvider(_relationshipId).future,
  ...
  group('CoachChatController.retryFailedMessages', () {
    test(
      'retrying a failed message on success moves it to confirmed',
      () async {
  ...
  group('CoachChatController.clearError', () {
    test(
      'clearError removes error from state without resetting messages',
      () async {
        when(
          () => coachService.sendChatMessage(
            coachUserId: any(named: 'coachUserId'),
    ...
    test(
      ...
      () async {
        ...
        final notifier = container.read(
          coachChatControllerProvider(_relationshipId).notifier,
        );
