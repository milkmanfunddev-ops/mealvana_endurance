...
- [x] Plans and Continue stay on screen through the whole scroll (screen widget test).
- [x] Annual shows "save 33%" and $16.67 a month on default, $8.33 a month on founding, computed from fake store prices (screen widget test).
- [x] The ⋯ menu lists exactly Restore, Manage (only with a subscription), Sign out and Delete account; no Redeem code (screen widget test).
- [x] No close button for a never-subscribed account.
- [x] Goldens of the full-screen paywall, light and dark, replace paywall_light, paywall_dark and paywall_onboarding_dark.
- [x] Checked on the dev simulator; Delete account still reachable for review.
