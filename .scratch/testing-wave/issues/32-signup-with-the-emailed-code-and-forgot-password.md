...
- [ ] Signs up at `lee+e2e-32-<UTC time>@rightpathprogramming.com`, logged in the credentials file at once.
- [ ] The code email arrives (the Gmail tool, `from:support@mealvana.io`); its arrival time, subject and sender are in the run notes, and Resend's log agrees if the agent can reach it. A missing or late email (over 2 minutes) is a Finding.
- [ ] A wrong code, an expired or reused code, and Resend code are each tried once and their result recorded; the right code opens the paywall.
- [ ] Before the code is entered, `auth.users.email_confirmed_at` is empty by SQL; after, it is set.
- [ ] Forgot password from the login screen: the email arrives, its link or code works on the simulator (Safari counts), the new password signs in and the old one is refused.
