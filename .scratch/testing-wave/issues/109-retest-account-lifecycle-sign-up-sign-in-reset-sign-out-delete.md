...
**Findings to retest:** 32-003 (after a reset, `auth.sessions` holds only the new sign-in). A global sign-out revokes refresh tokens only, so the other device's current access token still reads PostgREST for up to an hour; judge 32-003 by `auth.sessions` and by the other device failing its next token refresh, not by a PostgREST read in that hour (wave 28 review), 22-005 (a leftover Kroger connection from the other environment shows and can be removed; test@test.com's production row, if it is still there).
