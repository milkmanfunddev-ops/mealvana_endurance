...
- [x] Seam test through the real `MealPlanController`: Ate it waits for the server ack, writes a meal log with source plan and the plan meal's id, and drops one serving; a failed ack changes nothing and says so; at zero servings Ate it is hidden.
- [x] Widget tests: a row tap opens the sheet (not the detail); the Recipe link opens the detail; the earlier plan view's rows behave as its own rules say (check `previous_plan_screen.dart` before changing `PlanList`).
