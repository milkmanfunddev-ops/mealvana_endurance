...
42. As a paying athlete, I want to cancel, keep full access until my period ends, and then meet the full-screen paywall while staying signed in, so that cancellation follows the rule in the SSOT.
...
45. As a Lapsed athlete, I want to sign in and land on the full-screen paywall with no close button, with Restore, Redeem code, Manage, Sign out and Delete account in its ⋯ menu, so that the Gate's closed answer is proven.
46. As a Lapsed athlete, I want my data to be all there again the moment I resubscribe, and the server to refuse my AI calls while I am Lapsed, so that nothing is lost and the server's own check is proven.
...
- **Parallelism for this feature.** Two tickets at a time, which overrides the general pool of three. One build at a time under a build lock. No full unit-test run while a wave is live. Measured on 09-23: 18 GB machine, about 38% reclaimable. Two running simulator apps with their consoles fit; two concurrent builds do not. Fix waves are the exception (Lee, 2026-09-25): fix tickets (33 on) run no simulator scenario and open the whole frontier at once, like any build wave.
...
- **Purchases.** On a simulator the dev debug build buys through RevenueCat's Test Store, which is free and instant and has no Apple or Google record. Apple's sandbox is covered by one ready-for-human ticket on Lee's iPhone, with an agent checking (mp-289).
- **Cancellation rule.** Adopted from the record, not re-decided: access stays open until the entitlement's end date, then the Gate answers closed. Closed is the same full-screen paywall a new account sees, with no close button; the account stays signed in, there is no read-only mode, no plan-ended bar and no write check, and the server refuses AI calls on its own. Data is kept and is all there again when Pro is back. Lee re-ruled this on 2026-09-23, after waves 4 and 5 had built a read-only mode, so the first cancellation run is expected to raise SSOT-conflict Findings against the built app (mp-457, mp-280).
...
- **What the paying and planning scenarios check against.** An Admin skips only the paywall screen and the server still checks the subscription on every paid and Vana call (mp-416). Codes work once per account and only a coach Code pairs an athlete (mp-535). Restore, Redeem code, Manage, Sign out and Delete account sit in the paywall's ⋯ menu (mp-494). Confirming a draft archives every other plan for that week (mp-241), and the server builds the shopping list at confirm and after every edit (mp-244).
