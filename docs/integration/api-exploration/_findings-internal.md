...
| S3 | **Final Surge client id *and secret* committed to git.** Both files are tracked and not gitignored, so the secret is in history. Rotating at the vendor is the only step that closes it. | `tool/final_surge_api_test.dart:23-24`, `test/integration/final_surge_api_test.dart:31` |
...
| C2 | **TP write-back defaults ON in code, spec says OFF.** `tpWritebackEnabled` returns `?? true`; spec says "Default: OFF (opt-in to avoid surprising users)". | `preferences_service.dart:37` vs `docs/tp_write/README.md:232` |
...
| D2 | `docs/vdot/README.md:107` states a `−7d/+14d` sync window; code is `−14d/+45d`. | `vdot_sync_service.dart:68-69` |
