...
| S3 | **Final Surge client id *and secret* committed to git.** Both files are tracked and not gitignored. | `tool/final_surge_api_test.dart:23-24`, `test/manual_live/final_surge_api_test.dart:31` |
...
| C2 | **TP write-back defaults ON in code, spec says OFF.** `tpWritebackEnabled` returns `?? true`; the spec says "Default: **OFF** (opt-in to avoid surprising users)". We write into athletes' TrainingPeaks calendars without opt-in. | `preferences_service.dart:37` vs `docs/integration/tp_write/README.md:232` |
...
| D2 | `docs/integration/vdot/README.md:107` states a `−7d/+14d` sync window; the code is `−14d/+45d`. | `vdot_sync_service.dart:68-69` |
