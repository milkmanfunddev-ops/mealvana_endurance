...
**Frozen — 2026-09-15.** These are the last figures the pipeline measured, kept as a
record rather than a live gauge. A Meal now shows one Dish photo or nothing
([ADR 0003](../adr/0003-a-meal-shows-a-dish-photo-or-nothing.md)), the scripts are
archived under `scripts/_archived/meal-images/`, and the commands below are not to be
run. What they counted — the Verdicts, the Tile bank, each Meal's Tile list and
`image_mode` — stays in the database on purpose.
...
```bash
node scripts/_archived/meal-images/09-image-report.mjs --write   # the measure
...
`node scripts/_archived/meal-images/09-image-report.mjs --write`; do not edit by hand._
