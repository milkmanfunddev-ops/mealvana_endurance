...
5. **Edge function pin support** — pin-aware plan generation. **Revised 2026-05-22 from the original "new v4 directory" approach to extending v3 in place**, because the algorithm-level change landed as Option B (optional `pinnedTemplateIds: Set<string>` param on shared functions, byte-identical no-pin path proven by 16 parity tests in `pre-workout-pinned.test.ts` and `during-template-pinned.test.ts`). Substep 5 now decomposes into:
   ...
   - **5b-followup (2026-05-22)** — smoke test surfaced that 5b's plumbing is **inert in the production client flow**. Plan-v3 only runs the pin-aware selector when `input.pre_run_selections` is empty (`before-phase.ts:164`). The actual client flow is:
     1. Client → `generate-macros-v4` → returns macros + `pre_run_selections` (selection happens here, line 157 of `macros-v4/index.ts`)
     2. Client → `generate-nutrition-plan-v3` with those selections → plan-v3 explodes/scaffolds them (skips its own pin-aware selector)
     
     So pins must also be wired into `generate-macros-v4`. 5b's plan-v3 path stays — it's still correct for the regenerate / direct-plan-call fallback. Actions:
     - Fetch pins once in `macros-v4/index.ts` via `fetchUserPinnedTemplateIds(supabase, input.device_id)` (reuse `pins.ts` from 5b — either lift to `_shared/` or import across functions).
     - Pass `pinned.beforePinIds` as 10th arg to `selectPreWorkoutFoods` (brick path at line 157 + non-brick path inside `calculateMacrosV4`).
     - Plumb `pinned.duringPinIds` through the during selector path in macros-v4 (mirrors 5b's during-phase wiring).
     - Surface `pin_decision` on the macros-v4 response shape so the client can read it from the macros call.
     - Verify `device_id` is in the macros-v4 input contract (if not, treat as separate client-change subtask).
   - **5c** — smoke test against dev: pin a Before formula → generate plan → confirm `pin_decision.used_pin = true` in response **and** the pinned template name appears in the user-facing plan.
