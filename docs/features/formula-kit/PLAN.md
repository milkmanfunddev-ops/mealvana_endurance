...
7. ✅ **Edge function pin support** — `ad9d50de`. `post-template-solver.ts` already in place from earlier work; this commit added pin-awareness end-to-end: `generateAfterPhase` accepts `afterPinnedTemplateIds` + `pinsActive`, `selectPostWorkoutTemplateCandidates` short-circuits when any in-scope pin exists (bypassing allergen / diet / food-availability filters per locked 2026-05-22 policy), `filterPinnedPostTemplatesInScope` helper exported for the `pin_set_size` analytics field. `pin_decision` emitted under sibling `after_pin_decision` key on the wire (After section currently rides as flat list, not an object). `initialAfterPinDecision` skeleton mirrors `initialDuringPinDecision`. Smoke-tested on dev 2026-05-26 — pin override fires, wire shape stable. Note: substep 9's #35 follow-up landed as part of this work's debrief.
8. ✅ **Pin UX (After)** — folded into substep 6 (`bf530730`). `PinToggleAfter` widget on `AfterFormulaCard` + after detail screen, `FormulaPinController.toggleAfter()` mutation, writes through `FormulaPinsRepository` (already supports `post_system` kind from substep 2's CHECK widening). No separate `AfterPinToggle` variant needed — the toggle is a thin shape mirror of `PinToggleDuring` parameterized on `TemplateKind.postSystem`.
9. ✅ **Banner row (After)** — `95cd3568`. `pin_banner_rows_builder.dart` now treats the After section as pinnable (synthesizes `noPinForScope` rows when no real decision is emitted) and routes `after_run` / `after` section ids to the "After" label. `nutrition_plan_mapper.dart` parses `after_pin_decision` sibling key on the wire and attaches it to `PlanSection` via copyWith. 18 banner-rows-builder tests inverted/extended for After symmetry; full pin test suite (63/63 Dart + 18/18 Deno) green.

**Substep 9 follow-up (2026-05-26, three commits) — honor-pin food-load bypass (#35 fix):** smoke test on user `a98cab76` exposed a stale `chocolate_milk` dislike row corrupting the pin honor path. Pinned Chocolate Milk Solo silently fell through to LP because the dislike filter stripped the component before render. Three-commit fix on `feat/formula-kit`:
- `aa652f11` — **Option B (primary):** `getTemplateFoodsForPhase` gains `pinnedComponentNames?: Set<string>` parameter and bypasses dislike / allergen / diet filters for those foods. `after-phase.ts` now loads templates first (sequential), derives component names from pinned post-workout templates via new `derivePinnedPostComponentNames` helper, and passes them to the food loader. Mirrors `getTemplateFoodsForDuringWithConstraints` PR 2 5c pattern. **Option A guard:** when `renderPostTemplatePortion` returns null while pin override fired, downgrade `pin_decision.used_pin: false` with new wire reason `pinned_template_unrenderable`. Mirror `PinFallthroughReason.pinnedTemplateUnrenderable` enum value added to Dart side; banner branches on `usedPin` boolean only so no UI copy change.
- `850a4d9d` — 18 Deno tests (`after-phase-pinned.test.ts`): `derivePinnedPostComponentNames` (6), `filterPinnedPostTemplatesInScope` (4), `selectPostWorkoutTemplateCandidates` pin override (8). Symmetric with `during-template-pinned.test.ts`.
- `13c8db66` — **During parity:** Option A guard mirrored into `during-phase.ts:513`. The During food-load bypass (Option B) was already in place from PR 2 5c; what was missing was the wire downgrade when `generateDuringPhaseTemplate` returns null for every pinned candidate. Belt-and-braces — should be unreachable in practice now.

Smoke test 2026-05-26: logs confirm `pinnedComponents=1`, `foods=10` (chocolate_milk added back), no `[TMPL-FILTER-DISLIKED] chocolate_milk` line, `after_template_render result=success`, `path=template`. Wire now claims `used_pin: true` honestly. Underlying #35 (write-path dual-write) + #31 (read-path stale cache) handed off to colleague — Formula Kit is now robust to those data inconsistencies.
10. ✅ **Telemetry** — landed as side-effects of substeps 6–9; this substep just verifies the audit. `formula_pinned` / `formula_unpinned` carry `phase: 'after'` via `_phaseForKind(postSystem)` (`formula_pin_controller.dart:162`). `plan_used_pin` / `plan_pin_fallthrough` carry `phase: 'after'` via `_phaseForSectionId('after_run')` (`macro_targets_controller.dart:2141`). The library-tab event (canonical name in code is `formula_phase_switched`, not `formula_library_tab_selected` as drafted in the original plan) handles all three phases via `setPhase` + `FormulaPhase.after.analyticsValue`. `pinned_filter_toggled`, `formula_detail_viewed`, and `formula_filter_applied` thread `phase: 'after'` the same way. **Dropped:** the originally-planned new `pin_decision` reason `no_post_template_matches_filters`. The honor-pin policy locked at substep 9 unconditionally bypasses filters when a pin is in scope, and `pin_decision` is only emitted when pins are supplied — so "every template fails filters" can never fire on this wire shape. A separate plan-quality signal would be the right shape if a consumer ever needs it; none do today. Doc fix: `FormulaLibraryController.setPhase` comment updated from "Before / During" to all three phases.
11. ✅ **Tests + smoke** — landed in scope agreed 2026-05-26. **UI:** `efe3cf10` adds `after_formula_card_test.dart` (9 tests covering name, travel-friendliness pill, C:P ratio meta pill, pin affordance, pinned state, onTap; 103/103 formula_kit suite green). **Edge function:** pin path covered by `after-phase-pinned.test.ts` (21 steps post-`d9222bb0`, was 18 — added 3 for `renderPostTemplatePortion` missing-component hard-fail). **Smoke (dev):** pinned "PB&J + Milk" → full 4-component portion rendered (Jam, Milk, Peanut Butter, Whole Wheat Bread), `after_template_render result=success`, `[TMPL-FOODS-AFTER] Widening: fetched 3 pinned-component foods out of phase/activity scope`. **Deferred (no precedent):** `post_workout_templates_repository_test` (no during/before repo unit tests exist; integration paths exercise it) + integration test for pin → banner round trip (`integration_test/` has only one non-legacy test today; high cost, low marginal value over the live smoke). **Bug discovered + fixed mid-substep:** `d9222bb0` widens the `template_foods` SQL fetch for pinned components that fall outside the phase's category/activity scope, and hard-fails `renderPostTemplatePortion` when any required component is missing — replaces the silent skip that shipped 1/3 of a pinned template while claiming `result=success`. **Follow-up:** #36 filed for the no-pin half of the same architectural gap (After templates with pantry components — peanut_butter, jam, milk, granola, mixed_berries — are categorized for `before_run` not `after_run` and so are unreachable in the no-pin path).
