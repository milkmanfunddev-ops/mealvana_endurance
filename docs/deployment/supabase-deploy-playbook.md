...
| P3c | **Sandbox trial run on both stores — meal-planning releases** (mp-270, mp-289). By hand on physical devices, never in CI: `scripts/sandbox-trial-wizard.sh`, logs in `docs/release/sandbox-trial-runs/` committed with the release. See that folder's README for what counts | a `## Result: GREEN` store-checks log (Claude's part, mp-463) and a GREEN log for ios **and** android; no green run on both stores → no meal-planning release |
...
4b. **UI-bearing feature branches run `/device-sweep` before merging to `develop`.** The merge
   ...
   as "verify locally instead of pushing to find out", applied to pixels. (Added 2026-09-07,
   Xuan — the SE Brick-pill overflow shipped through green suites and was caught only by a
   populated-account sweep.)
...
| real-payload-corpus@v1 (provider_raw_payloads + first pg_cron sweep + corpus fns; no Drift change) | `../ops/docs/deploys/2026-09-real-payload-corpus.md` | LANDED 2026-09-20 (qa main d5d6f15, app 47425562 on release/1.27.0); prod half + flows seeding open |
