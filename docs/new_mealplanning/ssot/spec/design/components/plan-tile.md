...
**Status: RECORDED v1 (Lee, 2026-09-03) — PROPOSED, awaiting Xuan.** Extracted from `PlanTile` / `PlanList` /
`SwipeRow` / `TileSheet` (prototype `plan.tsx`, `swipe-row.tsx`) and the Dart tile (plan-tab-v2 decisions,
2026-08-31).
...
**Plan-tab writes carry no conversation scope** — they land on the week's active plan (P-2).
