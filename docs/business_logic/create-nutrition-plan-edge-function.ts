...
serve(async (req) => {
  ...
  try {
    ...
    // Get both liked and willing-to-try foods (prioritize liked foods)
    ...
    const preferredFoods = [...likedFoods, ...willingToTryFoods]; // Liked foods come first

    // Get all foods from database with new structured serving fields
...
// FOOD SELECTION LOGIC
async function selectFoodsForPlan({ calculations, foods, likedFoods, willingToTryFoods, preferredFoods, distanceMiles, durationHours }: {
...
function selectFoodsForCategory({ foods, likedFoods, willingToTryFoods, preferredFoods, carbTarget, category }: {
  foods: FoodItem[];
  likedFoods: string[];
  willingToTryFoods: string[];
  preferredFoods: string[];
  ...
  // Create food priority order: liked > willing-to-try > others
  const likedFoodsInCategory = foods.filter(f => likedFoods.includes(f.name));
  const willingToTryFoodsInCategory = foods.filter(f => willingToTryFoods.includes(f.name));
  ...
  console.log(`${category} food selection:`, {
    total_foods: foods.length,
    liked_count: likedFoodsInCategory.length,
    willing_to_try_count: willingToTryFoodsInCategory.length,
    other_count: otherFoods.length,
...
// UPDATED: Using structured serving data instead of hardcoded parsing
