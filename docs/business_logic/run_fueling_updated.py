...
class RunFuelOutput:
    ...
    post_run_carbs_g: int
    post_run_protein_g: int
    post_run_water_ml: int
    post_run_sodium_mg: int
...
def compute_run_fueling(params: dict) -> RunFuelOutput:
    ...
    post_carbs_g = clamp(round(weight_kg * 1.0), 30, 140)
    post_protein_g = clamp(round(weight_kg * 0.3), 18, 45)
    post_water_ml = clamp(round(during_water_rate_ml_h * 1.25), 500, 1200)
    post_sodium_mg = 500  # Fixed value without environmental factors
    ...
    return RunFuelOutput(
        ...
        post_run_carbs_g=post_carbs_g,
        post_run_protein_g=post_protein_g,
        post_run_water_ml=post_water_ml,
        post_run_sodium_mg=post_sodium_mg,
    )
...
if __name__ == '__main__':
    ...
    print()
    print("Post-run recovery nutrition:")
    print(f"  Carbs: {result.post_run_carbs_g}g")
    print(f"  Protein: {result.post_run_protein_g}g")
    print(f"  Water: {result.post_run_water_ml}ml")
    print(f"  Sodium: {result.post_run_sodium_mg}mg")