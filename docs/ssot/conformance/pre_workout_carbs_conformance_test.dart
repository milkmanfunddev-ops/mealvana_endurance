...
void main() {
    ...
    for (final v in vectors) {
      ...
      test('[$status] $id', () {
        ...
        void chkG(String key, double actual) {
          ...
          expect(actual, closeTo((exp[key] as num).toDouble(), tolG),
        ...
        expect(out.targetBasis, exp['targetBasis'], reason: '$id: targetBasis. $why');

        // --- the tiers array, walked in order -------------------------------
        final expTiers = (exp['tiers'] as List).cast<Map<String, dynamic>>();
        expect(out.tiers, isNotNull, reason: '$id: tiers is never null (empty when fasted)');
        expect(out.tiers.length, expTiers.length,
            reason: '$id: tier COUNT — expected ${expTiers.map((t) => t['tier']).toList()}, '
                'got ${out.tiers.map((t) => t.tier).toList()}. $why');
        expect(out.tiers.map((t) => t.tier).toList(), expTiers.map((t) => t['tier']).toList(),
        ...
        for (var k = 0; k < expTiers.length; k++) {
          ...
          expect(at.carbsG, closeTo((et['carbsG'] as num).toDouble(), tolG), reason: '$w: carbsG');
          expect(at.rangeLowG, closeTo((et['rangeLowG'] as num).toDouble(), tolG),
              reason: '$w: rangeLowG');
          expect(at.rangeHighG, closeTo((et['rangeHighG'] as num).toDouble(), tolG),
              reason: '$w: rangeHighG');
        ...
        expect(sum, closeTo(out.carbsG, tolG),
            reason: '$id: sum(tiers[].carbsG)=$sum must equal carbsG=${out.carbsG} (inv 1)');
      });
    }
  });
  ...
  group('pre-workout carbs invariants (property, grid 0..240 x BW 30..160)', () {
      ...
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          for (final dur in <double>[45, 60, 90]) {
            ...
            expect(sum, closeTo(o.carbsG, kExact),
                reason: 'BW=$bw t=$t dur=$dur: tiers sum $sum != carbsG ${o.carbsG}');
          }
        }
      }
    });
    ...
    test('inv 2 — carbsLowG <= carbsG <= carbsHighG', () {
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          for (final dur in <double>[45, 60, 90]) {
            final o = calc(bw, t, durationMin: dur);
            expect(o.carbsLowG <= o.carbsG + kExact, isTrue,
                reason: 'BW=$bw t=$t dur=$dur: low ${o.carbsLowG} > ${o.carbsG}');
            expect(o.carbsG <= o.carbsHighG + kExact, isTrue,
                reason: 'BW=$bw t=$t dur=$dur: ${o.carbsG} > high ${o.carbsHighG}');
          }
        }
      }
    });
    ...
    test('inv 4 — containment in the cited box: 1.0 <= carbsG/BW <= 4.0 for 60 <= t <= 240', () {
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid.where((t) => t >= 60 && t <= kWindowMax)) {
          final perKg = calc(bw, t).carbsG / bw;
          expect(perKg >= 1.0 - kExact, isTrue,
              reason: 'BW=$bw t=$t: $perKg g/kg is under Thomas 2016\'s 1 g/kg floor');
          expect(perKg <= 4.0 + kExact, isTrue,
              reason: 'BW=$bw t=$t: $perKg g/kg breaches Thomas 2016\'s 4 g/kg ceiling');
        }
      }
    });

    test('inv 5 — shares are EXACT (1e-9): 60/30/10 above 120, 75/25 above 30, 100 below', () {
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          final o = calc(bw, t);
          ...
          if (t >= kTierMealMin) {
            expect(tierNamed(o, 'meal')!.carbsG, closeTo(kShareMeal * total, kExact),
                reason: '$where: meal share != 0.60');
            expect(tierNamed(o, 'snack')!.carbsG, closeTo(kShareSnack * total, kExact),
                reason: '$where: snack share != 0.30');
            expect(tierNamed(o, 'top_off')!.carbsG, closeTo(kShareTopoff * total, kExact),
                reason: '$where: top_off share != 0.10');
          } else if (t >= kTierTopoffMax) {
            expect(tierNamed(o, 'snack')!.carbsG, closeTo(kShareSnackNm * total, kExact),
                reason: '$where: no-meal snack share != 0.75');
            expect(tierNamed(o, 'top_off')!.carbsG, closeTo(kShareTopoffNm * total, kExact),
                reason: '$where: no-meal top_off share != 0.25');
          } else {
            expect(tierNamed(o, 'top_off')!.carbsG, closeTo(total, kExact),
                reason: '$where: the top-off must carry 100% below TIER_TOPOFF_MAX');
          }
        }
      }
    });

    test('inv 6 — tier membership is exactly: meal iff t>=120, snack iff t>=30, top_off always',
        () {
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          final o = calc(bw, t);
          ...
          expect(names.contains('meal'), t >= kTierMealMin, reason: '$where: meal membership');
          expect(names.contains('snack'), t >= kTierTopoffMax, reason: '$where: snack membership');
          expect(names.contains('top_off'), isTrue,
              reason: '$where: the top-off tier is ALWAYS emitted (zero-valued at t=0)');
          // Ordered furthest-out first, no duplicates.
          expect(names.toSet().length, names.length, reason: '$where: duplicate tier');
          final rank = <String, int>{'meal': 0, 'snack': 1, 'top_off': 2};
          for (var k = 1; k < names.length; k++) {
            expect(rank[names[k]]! > rank[names[k - 1]]!, isTrue,
    ...
    test('inv 7 — boundary steps at t=120 and t=30 are pinned; carbsG itself is continuous', () {
      const eps = 1e-5;
      for (final bw in kBodyWeights) {
        ...
        expect((above120.carbsG - below120.carbsG).abs() < 1e-4 * bw, isTrue,
            reason: 'BW=$bw: carbsG is discontinuous at t=120 '
                '(${below120.carbsG} -> ${above120.carbsG})');
        expect(tierNamed(above120, 'meal'), isNotNull, reason: 'BW=$bw: meal opens at t=120');
        expect(tierNamed(below120, 'meal'), isNull, reason: 'BW=$bw: no meal below t=120');
        expect(tierNamed(above120, 'snack')!.carbsG,
            closeTo(kShareSnack * above120.carbsG, kExact),
            reason: 'BW=$bw: snack is 0.30 at t=120');
        expect(tierNamed(below120, 'snack')!.carbsG,
            closeTo(kShareSnackNm * below120.carbsG, kExact),
            reason: 'BW=$bw: snack STEPS to 0.75 just below t=120 — pin it, do not smooth it');
        ...
        expect((above30.carbsG - below30.carbsG).abs() < 1e-4 * bw, isTrue,
            reason: 'BW=$bw: carbsG is discontinuous at t=30 '
                '(${below30.carbsG} -> ${above30.carbsG})');
        expect(tierNamed(above30, 'snack'), isNotNull, reason: 'BW=$bw: snack opens at t=30');
        expect(tierNamed(below30, 'snack'), isNull, reason: 'BW=$bw: no snack below t=30');
        expect(tierNamed(above30, 'top_off')!.carbsG,
            closeTo(kShareTopoffNm * above30.carbsG, kExact),
            reason: 'BW=$bw: top-off is 0.25 at t=30');
        expect(tierNamed(below30, 'top_off')!.carbsG, closeTo(below30.carbsG, kExact),
            reason: 'BW=$bw: top-off STEPS to 100% just below t=30 — pin it');
      }
    });

    test('inv 8 — isFasted returns zeros and an EMPTY tiers array, never null', () {
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          for (final dur in <double>[45, 90]) {
            ...
            expect(o.tiers, isEmpty, reason: '$where: tiers must be EMPTY (distinct from t=0)');
      ...
      expect(zero.tiers.length, 1, reason: 't=0 emits the top_off tier carrying 0, not []');
      expect(zero.tiers.single.tier, 'top_off');
      expect(zero.targetBasis, 'design_choice',
      ...
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          for (final x in calc(bw, t).tiers) {
            expect(x.composition, isNotEmpty,
                reason: 'BW=$bw t=$t: tier ${x.tier} has no composition tag');
            expect(x.composition, x.tier,
                reason: 'BW=$bw t=$t: composition must mirror tier (VALUES are owned by '
    ...
    test('inv 11 — tier windows are +/-12.5%, and in design_choice they sum to the plan band', () {
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          for (final dur in <double>[59, 90]) {
            ...
            for (final x in o.tiers) {
              expect(x.rangeLowG, closeTo(x.carbsG * (1 - kTierTol), kExact),
                  reason: '$where: ${x.tier} rangeLowG != carbsG*0.875');
              expect(x.rangeHighG, closeTo(x.carbsG * (1 + kTierTol), kExact),
                  reason: '$where: ${x.tier} rangeHighG != carbsG*1.125');
              expect(x.rangeLowG <= x.carbsG + kExact, isTrue, reason: '$where: ${x.tier} low');
              expect(x.carbsG <= x.rangeHighG + kExact, isTrue, reason: '$where: ${x.tier} high');
            }
            if (o.targetBasis == 'design_choice') {
              // One tolerance used everywhere => the container holds its contents EXACTLY.
              final sumLow = o.tiers.fold<double>(0, (a, x) => a + x.rangeLowG);
              final sumHigh = o.tiers.fold<double>(0, (a, x) => a + x.rangeHighG);
              expect(sumLow, closeTo(o.carbsLowG, kExact),
                  reason: '$where: sum(rangeLowG)=$sumLow != carbsLowG=${o.carbsLowG}');
              expect(sumHigh, closeTo(o.carbsHighG, kExact),
                  reason: '$where: sum(rangeHighG)=$sumHigh != carbsHighG=${o.carbsHighG}');
            } else {
              // Cited regime: the plan band is Thomas's [1*BW, 4*BW].
              expect(o.carbsLowG, closeTo(1.0 * bw, kExact), reason: '$where: cited low');
              expect(o.carbsHighG, closeTo(4.0 * bw, kExact), reason: '$where: cited high');
            }
          }
        }
      }
    ...
    test('targetBasis flips on BOTH legs: workoutDurationMin at 59.999/60 and t at 59.999/60', () {
      for (final bw in kBodyWeights) {
        expect(calc(bw, 180, durationMin: 59.999).targetBasis, 'design_choice',
            reason: 'BW=$bw: a sub-60-min workout leaves Thomas 2016\'s scope');
        expect(calc(bw, 180, durationMin: 60).targetBasis, 'evidenced_band',
            reason: 'BW=$bw: 60 min is INCLUSIVE');
        expect(calc(bw, 59.999, durationMin: 90).targetBasis, 'design_choice',
            reason: 'BW=$bw: a sub-1h lead time leaves the cited window');
        expect(calc(bw, 60, durationMin: 90).targetBasis, 'evidenced_band',
  ...
  group('cross-spec pin', () {
    test('TIER_MEAL_MIN == pre-workout-hydration.T_REF (both 120.0)', () {
      ...
      expect(mealMinObserved, kTierMealMin, reason: 'carbs TIER_MEAL_MIN drifted off 120.0');
      expect(tRefObserved, kTierMealMin,
          reason: 'hydration T_REF ($tRefObserved) diverged from carbs TIER_MEAL_MIN '
