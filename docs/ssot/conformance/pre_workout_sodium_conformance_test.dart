...
void main() {
    ...
    for (final v in vectors) {
      ...
      test('[$status] $id', () {
        ...
        void chkNull(String key, int? actual) {
          ...
          expect(e[key], isNull,
              reason: '$id: the vector file expects a NUMBER for $key, but SSOT v3 ratifies '
                  'null. Fix the vector, not this assertion.');
          expect(actual, isNull,
              reason: '$id: $key must be strictly null, got $actual. '
  ...
  group('pre-workout sodium invariants (property)', () {
    test('all three sodium fields are strictly null across the whole input domain', () {
          ...
          for (final check in kAllChecks) {
            ...
            for (final dur in <double>[45, 60, 90, 180]) {
              for (final temp in <double?>[null, 5, 22, 32]) {
                ...
                final where = 'BW=$bw t=$t dur=$dur temp=$temp check=$check '
                    '(gate=${o.gateTriggered})';
                expect(o.sodiumMg, isNull, reason: '$where: sodiumMg must be null, not 0');
                expect(o.sodiumLowMg, isNull, reason: '$where: sodiumLowMg must be null, not 0');
                expect(o.sodiumHighMg, isNull, reason: '$where: sodiumHighMg must be null, not 0');
              }
            }
          }
        }
    ...
    test('sodium is null in every tier of every plan, not just at the plan level', () {
      ...
      for (final bw in kBodyWeights) {
        for (final t in kLeadGrid) {
          for (final check in kAllChecks) {
            ...
            expect(o.tiers, isNotEmpty,
                reason: 'BW=$bw t=$t check=$check: a non-gated plan always has tiers');
