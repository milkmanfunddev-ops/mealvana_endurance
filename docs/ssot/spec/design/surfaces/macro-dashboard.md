...
| S-5 | Scope guard (Xuan): no phase indicator, no safety states, no sodium on this surface — deferred, not forgotten. **AMENDED — RULED (Xuan, 2026-09-24, carb-loading interview, post-ratification addition):** one exception — the carb-loading **LOAD face** (a fuelling-phase face) is admitted, replacing the All-lens energy-card face on days inside a created carb-loading plan (`energy-card.md` v2 amendment holds the face contract; `spec/fueling/carb-loading.md` the numbers; intake `2026-09-24-carb-loading-amend-a-dashboard-scope-guard.md`). Safety states and sodium stay deferred |
