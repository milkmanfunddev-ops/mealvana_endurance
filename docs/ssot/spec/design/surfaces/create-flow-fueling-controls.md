...
| # | Contract |
|---|---|
| CF-1 | **PRE-RUN FUELING WINDOW stepper**: −/+ in 15-min steps; label `N HOUR[S] M MIN`. Its DEFAULT is §3a's table (incl. the early-start and race rules) — not the retired formula. Its MAXIMUM is the ruled clamp: `min(table cap, time-until-start)`, floor 15 min; values above the clamp are unreachable, not disabled-but-visible. A manual change persists (`preRunMinutesManuallySet`) and wins within the clamp. |
| CF-2 | When the clamp binds (short-notice plan), the stepper opens AT the clamp and stepping up is inert — the athlete sees the real ceiling, never a window implying a feeding in the past. |
