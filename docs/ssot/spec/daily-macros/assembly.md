...
**Status: RECORDED — awaiting ratification** (2026-07-28). Source: Notion assembly formulas
...
4.  TODAY'S SESSIONS                                         [session-demand.md]
    if 2+ sessions:  { session_carb, prot_bump } = multiSessionCarbCompound(...)  # F19
...
8.  PHASE MODIFIERS (multiplicative)                         # F10
    ...
    (mods.fat_mod is returned but never applied — see Q-004)
...
12. RETURN (round here, and only here — see R1)
    { carb_g, prot_g, fat_g, tdee, rmr, session_kcal,
      neat_kcal, tef_kcal, mode, ea, ea_status, sources, delta }
```
...
- **EA gate last** (11): it needs `fat`, which needs TDEE, which needs the final carb and protein.
  This is also why the override in step 11 sits outside every clamp and every recompute — the
  known consequence registered as [Q-006](OPEN-QUESTIONS.md#q-006) and
...
|---|---|---|---|---|
| none (rest) | 300 | 115 | 80 † | 2385 |
| run 1.5 hr IF 0.74 | 369 | 130 | 177 | 3590 |
| bike 4.0 hr IF 0.72 | 498 | 130 | 262 | 4873 |
| strength 1.0 hr IF 0.70 | 340 | 138 | 92 | 2735 |
| bike 1.25 hr IF 0.93 | 416 | 130 | 166 | 3682 |

† Reconciles only with unrounded intermediates — see [R1](README.md#cross-cutting-rules) and
[Q-001](OPEN-QUESTIONS.md#q-001). The strength row is the decisive case: unrounded protein 137.7
gives fat 92, rounded 138 gives 91.
...
this SSOT should be tighter than this** — the formulas are deterministic, and a ±15 % fat band
would hide most of the rounding questions in the register. Recommend exact-match vectors with the
rounding rule pinned, once [Q-001](OPEN-QUESTIONS.md#q-001) is ruled on.
