...
**Engine:** B — daily macros. Primary implementation: the `calculate-daily-macros` edge function;
vectors run against it, not `OfflineMacroCalculator`. **A client-side Dart mirror exists —
corrected 2026-08-17 (intake erratum `intake/2026-08-17-readme-dart-mirror-stale.md`; the original
"no Dart mirror" claim was true when recorded 2026-07-28, stale since the onboarding redesign):**
`DailyBaselineCalculator` (`app/lib/features/nutrition_plan/application/daily_baseline_calculator.dart`)
powers the offline, account-less onboarding plan preview and mirrors F1 (RMR), F2 (baseline), the
step-9 clamps, F3 (zone→IF), F4 (session cost), F5 (carb demand incl. Q-003 strength flat rate),
F15 (iterative TEF, uncapped residual) and assembly step 10b (Q-014 fat cap, applied in
`PlanPreviewService`). Its conformance contract is the shared parity fixture
`app/test/features/onboarding/fixtures/plan_preview_parity.json` — generated by
`app/supabase/functions/calculate-daily-macros/tools/generate-parity-fixtures.ts` from the
vector-pinned TS formulas (never hand-edited), consumed by both `parity-fixtures.test.ts` (Deno)
and `daily_baseline_calculator_test.dart` (Dart). **Release coupling — RULED (Xuan, 2026-08-17,
post-ratification addition):** preview and deployed engine must ship the same formula generation,
or new users see an onboarding preview that disagrees with their post-onboarding plan.
