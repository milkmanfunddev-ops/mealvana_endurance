...
## 0. Dimensions (retrofit 2026-08-20 — see `docs/test-plan-dimensions.md`)

Added after the fact: the mark-done-teleport bug (Q-D7) lived in an unexamined cell of this
matrix for four days while every listed chain was green. Axes × the card gestures:

| Contract | past day | current day | future day | during recompute | wire round-trip |
|---|---|---|---|---|---|
| G1 mark-done | ✅ gestures `g1` (v3) | ✅ `g1` | ✅ `g1_future_day_not_offered` | open — freeplay only | ✅ Patrol + T15 probe |
| G2 undo | ✅ `g2` (derived SKIPPED) | ✅ `g2` | ✅ (legacy confirmed) | open | ✅ Patrol |
| G4/G5 skip/unskip | ✅ `g4`/`g5` | ✅ | ✅ (skip offered) | ⚠ pinned only as a Patrol retry-race | ✅ Patrol |
| G6 sync-beats-skip | ✅ `g6` | ✅ | excluded — sync implies the session happened | n/a | ✅ **T15 probe-live (deployed garmin-push)** |
| Profile save (Q-016) | ✅ never touched | ✅ invalidated | ✅ invalidated | open | open — targets round-trip unpinned (now I7: the 70-kg bug lived exactly here) |

Open cells above are the current debt, tracked here rather than silent. Concurrency cells are
exercised by `/sim-explore` free play until pinned properly.
...
| I4 | Every number on the dashboard maps to a spec field; no surface disagrees with another on the same quantity | intraday-display P-3 / surface S-3 | ⬜ — now CONCRETE (2026-08-20, after the 70-kg/three-surfaces bugs): for a seeded athlete, card + Active Energy sheet kcal == `F4(profile weight, resolved IF)` and == the create-flow Total Burn; one IF source (zones when present) across all surfaces. Ops refs: `2026-08-20-session-kcal-three-surfaces-disagree.md`, `-dashboard-weight-fallback-70kg.md` |
...
| I7 | **Inputs survive the seams (added 2026-08-20):** every field the engine writes into a cached target row (incl. `calculation_input`'s `weight_kg`) is readable back through the repository mapper — write via `daily_macro_service`, read via `daily_macro_targets_repository`, assert field-for-field. No display-layer `??`-default may silently stand in for an engine input; a missing input fails loudly or flags the value as estimated | this plan (lesson of the 70-kg bug: the mapper omission was invisible because `?? 70.0` made it non-crashing) | ⬜ seam round-trip test |
| I8 | **Weight diversity (added 2026-08-20):** display-layer kcal assertions run at 50 / 75 / 95 kg, never only the 75-kg reference athlete — extends the spec's own 60:75:90 ratio discipline (session-demand.md F4) above the engine layer. A silent ~70-kg substitution is invisible at reference weight (~7%) and +40% at 50 kg | session-demand.md F4 (cost linear in weight) | ⬜ parameterize assembler/widget kcal tests |
