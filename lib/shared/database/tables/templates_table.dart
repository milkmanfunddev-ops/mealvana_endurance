...
class TemplatesTable extends Table {
  ...
  TextColumn get mealType => text().withDefault(const Constant('snack')).named('meal_type')();
  ...
  RealColumn get totalCarbsG => real().withDefault(const Constant(0)).named('total_carbs_g')();
  RealColumn get totalProteinG => real().withDefault(const Constant(0)).named('total_protein_g')();
  RealColumn get totalFatG => real().withDefault(const Constant(0)).named('total_fat_g')();
  RealColumn get totalSodiumMg => real().withDefault(const Constant(0)).named('total_sodium_mg')();
  RealColumn get totalFluidMl => real().withDefault(const Constant(0)).named('total_fluid_ml')();
  IntColumn get totalCalories => integer().withDefault(const Constant(0)).named('total_calories')();

  /// Validation status: unvalidated/validated/needs_adjustment/failed
  TextColumn get validationStatus => text().withDefault(const Constant('unvalidated')).named('validation_status')();
  ...
  TextColumn get foodNames => text().withDefault(const Constant('[]')).named('food_names')();
