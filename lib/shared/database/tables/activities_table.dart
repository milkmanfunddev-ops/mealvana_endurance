...
class ActivitiesTable extends Table {
  ...
  TextColumn get userId =>
      text().named('user_id')(); // FOREIGN KEY to user_profiles.id

  TextColumn get activityType =>
      text().named('activity_type')(); // 'running', 'cycling', 'swimming'
  TextColumn get title => text()();
  DateTimeColumn get scheduledDateTime =>
      dateTime().named('scheduled_date_time')(); // TIMESTAMP
  ...
  TextColumn get intensityLevel => text().nullable().named(
    'intensity_level',
  )(); // 'easy', 'moderate', 'hard', 'race'

  // Cycling-specific parameters
  RealColumn get cyclingSpeedMph =>
      real().nullable().named('cycling_speed_mph')();
  TextColumn get cyclingTerrain => text().nullable().named(
    'cycling_terrain',
  )(); // 'flat', 'rolling', 'hilly'
  TextColumn get cyclingIndoorOutdoor => text().nullable().named(
    'cycling_indoor_outdoor',
  )(); // 'indoor', 'outdoor'
  IntColumn get cyclingElevationGainFt =>
      integer().nullable().named('cycling_elevation_gain_ft')();
  TextColumn get cyclingSessionGoal => text().nullable().named(
    'cycling_session_goal',
  )(); // 'endurance', 'tempo', 'intervals'

  // Swimming-specific parameters
  IntColumn get swimmingPacePer100mSeconds =>
      integer().nullable().named('swimming_pace_per_100m_seconds')();
  TextColumn get swimmingPoolOrOpenWater => text().nullable().named(
    'swimming_pool_or_open_water',
  )(); // 'pool', 'open_water'
  RealColumn get swimmingWaterTempC =>
      real().nullable().named('swimming_water_temp_c')();

  // Shared intensity and timing
  TextColumn get intensityTarget => text().nullable().named(
    'intensity_target',
  )(); // 'zone_1', 'zone_2', 'rpe_3', etc.
  ...
  IntColumn get intensityZ1Z2Pct => integer().nullable().named(
    'intensity_z1_z2_pct',
  )(); // Conversational zone (Z1-Z2)
  IntColumn get intensityZ3Z4Pct =>
      integer().nullable().named('intensity_z3_z4_pct')(); // Tempo zone (Z3-Z4)
  IntColumn get intensityZ5Pct =>
      integer().nullable().named('intensity_z5_pct')(); // All-out zone (Z5+)

  // Reminder settings
  BoolColumn get reminderEnabled =>
      boolean().withDefault(const Constant(false)).named('reminder_enabled')();
  IntColumn get reminderDaysBefore => integer().nullable().named(
    'reminder_days_before',
  )(); // 1-7 days before activity
  TextColumn get reminderTimeOfDay =>
      text().nullable().named('reminder_time_of_day')(); // Format: "HH:mm"
  BoolColumn get reminderRecurring => boolean()
      .withDefault(const Constant(false))
      .named('reminder_recurring')();
  ...
  TextColumn get syncedFromProvider => text().nullable().named(
    'synced_from_provider',
  )(); // 'final_surge', 'training_peaks', etc.
  TextColumn get providerWorkoutId =>
      text().nullable().named('provider_workout_id')(); // External workout ID
  TextColumn get providerWorkoutUrl => text().nullable().named(
  ...
  BoolColumn get needsNutritionRefresh => boolean()
      .withDefault(const Constant(false))
      .named(
        'needs_nutrition_refresh',
      )(); // Flag when schedule changed significantly
  DateTimeColumn get providerDeletedAt => dateTime().nullable().named(
    'provider_deleted_at',
  )(); // When provider removed this workout (soft delete)
  DateTimeColumn get providerScheduledAt => dateTime().nullable().named(
    'provider_scheduled_at',
  )(); // Original provider schedule for change detection
  DateTimeColumn get scheduleChangedAt => dateTime().nullable().named(
    'schedule_changed_at',
  )(); // When we last detected a schedule change
  ...
  RealColumn get paceMinMinutesPerMile =>
      real().nullable().named('pace_min_minutes_per_mile')();
  RealColumn get paceMaxMinutesPerMile =>
      real().nullable().named('pace_max_minutes_per_mile')();
  ...
  IntColumn get completionRating =>
      integer().nullable().named('completion_rating')(); // 1-5
  TextColumn get completionNotes =>
      text().nullable().named('completion_notes')();
  RealColumn get actualDistanceMiles =>
  ...
  TextColumn get nutritionPlanData =>
      text().nullable().named('nutrition_plan_data')();
  ...
  TextColumn get brickMetadata => text().nullable().named(
    'brick_metadata',
  )(); // JSONB in Supabase, TEXT in SQLite
  TextColumn get brickId => text().nullable().named(
    'brick_id',
  )(); // UUID reference to parent brick activity
  ...
  TextColumn get garminSummaryId =>
      text().nullable().named('garmin_summary_id')();

  // Garmin device model (e.g., "Forerunner 955") for brand-compliant attribution
  TextColumn get garminDeviceName =>
      text().nullable().named('garmin_device_name')();
