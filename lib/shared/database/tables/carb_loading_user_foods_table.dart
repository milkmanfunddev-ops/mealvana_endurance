...
class CarbLoadingUserFoodsTable extends Table {
  ...
  TextColumn get deviceId =>
      text().named('device_id')(); // Device that created this food
  TextColumn get userId =>
      text().named('user_id')(); // User ID (UUID) - references users.id
  TextColumn get clientFoodId => text().nullable().named(
    'client_food_id',
  )(); // Client-side ID for sync tracking
  TextColumn get name => text()(); // 'my_favorite_pasta'
  TextColumn get displayName =>
      text().named('display_name')(); // 'My Favorite Pasta (2 cups)'
  TextColumn get displayNamePlural => text().nullable().named(
    'display_name_plural',
  )(); // 'My Favorite Pasta (2 cups)'
  ...
  TextColumn get barcode =>
      text().nullable()(); // Barcode if scanned via barcode scanner
  TextColumn get sourceFoodId => text().nullable().named(
    'source_food_id',
  )(); // FK to foods.id (if imported from nutrition plan)
  TextColumn get sourceUserFoodId => text().nullable().named(
    'source_user_food_id',
  )(); // FK to user_foods.id (if imported from user's custom foods)
  ...
  BoolColumn get isDeleted => boolean()
      ...
      .named('is_deleted')(); // Soft delete to preserve history
  ...
  @override
  List<String> get customConstraints => ['CHECK (carbs_per_serving > 0)'];
}
