...
class CarbLoadingPlansTable extends Table {
  ...
  TextColumn get eventId => text().nullable().named(
    'event_id',
  )(); // FOREIGN KEY to events.id (nullable: can have standalone plans)
  ...
  IntColumn get dailyCarbTargetGrams =>
      integer().named('daily_carb_target_grams')();
  IntColumn get dailyCalorieTarget =>
      integer().nullable().named('daily_calorie_target')();

  // Plan metadata
  DateTimeColumn get generatedAt => dateTime().named('generated_at')();
  TextColumn get algorithmVersion =>
      text().withDefault(const Constant('v1.0')).named('algorithm_version')();

  // Completion tracking
  RealColumn get adherenceScore => real().nullable().named(
    'adherence_score',
  )(); // 0.0 to 1.0 based on logged meals
