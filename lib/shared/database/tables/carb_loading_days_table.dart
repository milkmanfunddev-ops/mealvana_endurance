...
class CarbLoadingDaysTable extends Table {
  ...
  IntColumn get dayNumber => integer().named('day_number')(); // 1, 2, 3, etc. (1 = first day, last number = race day - 1)
  ...
  RealColumn get breakfastPercent => real().withDefault(const Constant(0.25)).named('breakfast_percent')();
  RealColumn get morningSnackPercent => real().withDefault(const Constant(0.10)).named('morning_snack_percent')();
  RealColumn get lunchPercent => real().withDefault(const Constant(0.25)).named('lunch_percent')();
  RealColumn get afternoonSnackPercent => real().withDefault(const Constant(0.15)).named('afternoon_snack_percent')();
  RealColumn get dinnerPercent => real().withDefault(const Constant(0.20)).named('dinner_percent')();
  RealColumn get eveningSnackPercent => real().withDefault(const Constant(0.05)).named('evening_snack_percent')();
