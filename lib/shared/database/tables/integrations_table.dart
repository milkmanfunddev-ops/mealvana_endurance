...
class IntegrationsTable extends Table {
  ...
  DateTimeColumn get tokenExpiresAt =>
      dateTime().nullable().named('token_expires_at')();

  // Provider-specific athlete data (displayed in Settings)
  TextColumn get providerAthleteId => text().named('provider_athlete_id')();
  TextColumn get providerAthleteName =>
      text().nullable().named('provider_athlete_name')();
  TextColumn get providerAthleteEmail =>
      text().nullable().named('provider_athlete_email')();
  ...
  RealColumn get providerAthleteWeightKg =>
      real().nullable().named('provider_athlete_weight_kg')();
  TextColumn get providerAthleteBirthMonth => text().nullable().named(
    'provider_athlete_birth_month',
  )(); // "YYYY-MM" format
  TextColumn get providerAthleteGender =>
      text().nullable().named('provider_athlete_gender')(); // 'm' or 'f'
  RealColumn get providerAthleteBodyFatPct =>
      real().nullable().named('provider_athlete_body_fat_pct')();
  ...
  DateTimeColumn get lastSyncAt =>
      dateTime().nullable().named('last_sync_at')();
  TextColumn get lastSyncStatus => text().nullable().named(
    'last_sync_status',
