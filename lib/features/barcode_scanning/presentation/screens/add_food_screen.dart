...
class _AddFoodScreenState extends ConsumerState<AddFoodScreen> {
  ...
  Future<void> _saveSearchedFood(
    ...
    try {
      ...
      final barcode =
          foodItem.description?.replaceAll('Scanned from barcode ', '') ?? '';
      ...
      if (hasDuplicate) {
        ...
        if (mounted) {
          MealvanaSnackbar.showWarning(
            context,
            '${foodItem.name} is already in your foods',
          );
        }
        return;
      }
      ...
      final categoryNames = categoryIds.map((id) {
        switch (id) {
          ...
          case 3:
            return 'after_run';
          default:
            return 'before_run'; // Default fallback
        }
      }).toList();
  ...
  Future<void> _handleBarcodeScan() async {
    ...
    final result = await context.pushNamed(
      'barcode-scanner',
      extra: {'category': 'add_food', 'context': 'add_food'},
    );
