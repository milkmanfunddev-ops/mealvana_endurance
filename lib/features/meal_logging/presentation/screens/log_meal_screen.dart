...
class _LogMealScreenState extends ConsumerState<LogMealScreen> {
  ...
  void initState() {
    ...
    _diary = ref.read(diarySessionProvider)..open(logDate: widget.logDate);
  ...
  @override
  void dispose() {
    // `diary_closed`: dwell time and the meals that landed (counted by the
    // meal controller). A handed-off session closes with Review & Log.
    _diary.closeDiary();
...
class _AiTabState extends ConsumerState<_AiTab> {
  ...
  Future<void> _analyze() async {
    ...
    try {
      ...
      // Review & Log saves the meal after Log a Meal has closed, so it
      // closes the diary session too (testing-wave 24-002).
      ref.read(diarySessionProvider).handOff();
