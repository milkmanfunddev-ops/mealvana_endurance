...
class OnboardingController extends _$OnboardingController {
  ...
  ///
  /// Called once the account exists (mp-459): the draft has waited on the
  /// phone through onboarding and is written here under the signed-in uid.
  /// With no session the profile step throws and this returns false with the
  /// draft intact, so the caller can send the athlete back to sign up.
  /// [authProvider] - 'email', 'google', 'apple'
  Future<bool> saveAllOnboardingData({required String authProvider}) async {
    DebugLogger.info(
      '📦 Starting batch save of all onboarding data (authProvider: $authProvider)',
    );
    ...
    state = await AsyncValue.guard(() async {
      ...
      // delete-and-recreate upgrade path can restore a profile whose data
      // never reached Supabase (plan §7). Never fails the save.
      //
      ...
      // Onboarding data always goes to Supabase. The profile was written
      // under the signed-in uid, so `auth.uid()` == `users.id` and every RLS
      ...
      // caller's post-save upload — see PostOnboardingAuthScreen.
  ...
  ///
  /// Called by the post-onboarding auth screen *after* it has navigated on,
  /// so it never sits between the user and the app. Onboarding data always
  /// reaches Supabase.
  ///
  ...
  ///
  /// Onboarding runs with no session (mp-459), so a training app connected
  /// before sign-up writes its activities, events, integrations, etc. under
  /// the temporary user ID generated by ConnectTrainingController. This
  /// re-keys all of it onto the account once the profile exists.
  ///
  ...
  ///
  /// This row is what makes every dependent upload (activities, integrations,
  /// food preferences, formula pins) satisfy both its FK and its
