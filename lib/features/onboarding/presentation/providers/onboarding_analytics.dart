...
///
/// Deliberately not Mixpanel's `timeEvent()`: that emits a `$duration`
/// property rather than the `duration_sec` the funnel spec asks for.
///
/// A plain static rather than a provider, so the helper carries no Riverpod
/// coupling and reads identically on this release branch and on dev.
class OnboardingAnalytics {
  ...
  ///
  /// Callers must omit the property when this is null rather than send a zero,
  /// which would drag the median completion time down.
...
///
/// NOTE: this shipped 1.20 flow has SIX steps — it still collects food
/// preferences, which the dev line dropped on 2026-06-25. The funnel for this
/// build is therefore genuinely one step longer than dev's; don't "fix" the
/// difference by copying dev's list, or `step_index` will stop lining up with
/// the pages real users are looking at.
