...
class ActivityDetailScreen extends ConsumerStatefulWidget {
  ...
  final int? activityId; // For view mode
...
class _ActivityDetailScreenState extends ConsumerState<ActivityDetailScreen> {
  ...
  Future<void> _showDateTimePicker(DateTime currentDateTime) async {
    ...
    // Update the activity's scheduled date time
  ...
  Future<void> _showReminderSettingsSheet(ActivityDetailState state) async {
    ...
    // Track if reminder was saved
    bool reminderSaved = false;
    ActivityReminder? savedReminder;
    ...
    // Show success snackbar after bottom sheet closes
    if (mounted && reminderSaved) {
      final message = savedReminder == null
  ...
  Widget build(BuildContext context) {
    ...
    // Track nutrition plan changes for marking activity as changed
    activityDetailState.whenData((state) {
      if (state.nutritionPlan != null) {
        final currentPlanId = state.nutritionPlan!.id;
        // Count all food items across all sections
        final currentUpdateCount = state.nutritionPlan!.sections
        ...
        if (_lastPlanId == currentPlanId && _lastPlanUpdateCount != null) {
          if (currentUpdateCount != _lastPlanUpdateCount) {
            // Plan has changed - already marked as changed by controller methods
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          // Macro Targets Widget (if we have macro targets and nutrition plan)
            ...
            PlanContainer(
              ...
              onDeleteFood: (foodItemId, category) async {
                ...
                if (confirm == true) {
                  ...
                  await controller.deleteFoodItem(foodItemId, category);
                }
              },
              onUpdateQuantity: (foodItemId, category, newQuantity) async {
                ...
                await controller.updateFoodQuantity(foodItemId, category, newQuantity);
              },
            )
          else
            _buildNoPlanState(),
  ...
  String _formatWorkoutReference(targets_model.MacroTargets targets) {
    ...
    switch (targets.activityType) {
      ...
      case ActivityType.duathlon:
      case ActivityType.multisport:
        return '$distance miles'; // Generic distance for multi-sport
    }
  }
