...
/// Renders the always-visible TL;DR section for carb transparency.
///
...
class CarbTldrSection extends StatelessWidget {
  const CarbTldrSection({
    super.key,
    required this.data,
  });
  ...
  Widget build(BuildContext context) {
    ...
    if (data.isSwimZero) {
      return _buildSwimZeroState(context, primaryText, secondaryText, dimColor);
    }
            ...
            child: Column(
              ...
              children: [
                for (final line in data.tldrLines) ...[
                  ...
                  _buildFormulaLine(line, primaryText, secondaryText, dimColor),
                ],
              ],
            ),
          ),
        ...
        if (data.tldrTip != null) ...[
          ...
          Container(
            ...
            child: _buildRichBody(
              data.tldrTip!,
              AppColors.electrolyte.withValues(alpha: 0.85),
              primaryText,
              fontSize: 12.5,
              height: 1.55,
  ...
  Widget _buildFormulaLine(
    ...
    final spans = line.segments.map((seg) {
      return TextSpan(
        text: seg.text,
        style: _segmentStyle(seg.style, primaryText, secondaryText, dimColor),
      );
    }).toList();
