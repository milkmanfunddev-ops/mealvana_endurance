...
class ConnectTrainingController extends _$ConnectTrainingController {
  ...
  Future<int> importFinalSurgeWorkouts() async {
    ...
    state = AsyncData(state.value!.copyWith(isImporting: true, importProgress: 0.0));
    ...
    try {
      ...
      if (result.hasNewWorkouts && result.activities.isNotEmpty) {
        await _bulkPlanService.generatePlansForActivities(
          result.activities,
          onProgress: (progress) {
            state = AsyncData(state.value!.copyWith(importProgress: 0.5 + (progress.fraction * 0.5)));
          },
        );
      }
